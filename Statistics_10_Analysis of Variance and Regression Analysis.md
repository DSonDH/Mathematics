...
$$\bar X_i\sim N(\mu_i,\sigma^2/n_i), (SS_i/\sigma^2\sim\chi^2(n_i-1))$$
...
* 모평균 추정량:
    ...
    $$\hat\mu_{ij} = \bar X_{ij\cdot} = \frac{1}{n_{ij}}\sum_{k=1}^{n_{ij}} X_{ijk}$$
...
>
>$$ \hat\sigma^2_{\mathrm{MLE}} =\frac{1}{n}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^{n_{ij}}(X_{ijk}-\bar X_{ij\cdot})^2$$
>
...
$$Y=(X_{111},\dots,X_{11r},\ X_{121},\dots,\ X_{abr})^\top\in\mathbb R^{n},\quad n=rab$$
...
$$\mathrm{cgf}_Y(t) =\sum_{k=1}^\infty \frac{t^k}{k!}\,2^{k-1}(k-1)!(r+k\delta)$$
...
$$\exp\!\left(\frac{\delta t}{1-2t}\right) =e^{-\delta/2}\sum_{k=0}^\infty \frac{(\delta/2)^k}{k!}(1-2t)^{-k}$$
...
* 벡터/행렬로 쓰면
$$Y = X\beta + e,\quad e\sim N_n(0,\sigma^2 I),\quad \mathrm{rank}(X)=p+1,\quad \sigma^2>0$$
...
$$\widehat{c^\top\beta}=c^\top\hat\beta,\quad \hat\beta=(X^\top X)^{-1}X^\top Y$$

* 오차분산의 불편추정량
$$\hat\sigma^2=\frac{\|Y-X\hat\beta\|^2}{n-p-1} =\frac{(Y-X\hat\beta)^\top(Y-X\hat\beta)}{n-p-1}$$
