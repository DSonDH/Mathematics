...
$$\hat\mu_i = \bar x_i, \quad {\hat\sigma^2}^\Omega = \frac{1}{n}\sum_{i=1}^k\sum_{j=1}^{n_i}(x_{ij}-\bar x_i)^2 = \frac{SSW}{n}$$
...
\therefore \hat\sigma_0^2 = \frac{SSW+SSB}{n}, \quad {\hat\sigma^2}^\Omega = \frac{SSW}{n}$$

**(2) 최대가능도비 검정통계량**  
MLE를 로그가능도에 대입하면:

$$l({\hat\theta}^\Omega) = -\frac{n}{2}\log(2\pi{\hat\sigma^2}^\Omega) - \frac{n}{2}, \quad l(\hat\theta^0) = -\frac{n}{2}\log(2\pi\hat\sigma_0^2) - \frac{n}{2}$$

최대가능도비 검정통계량:

$$2\{l({\hat\theta}^\Omega) - l(\hat\theta^0)\} = n\log\left(\frac{\hat\sigma_0^2}{{\hat\sigma^2}^\Omega}\right) = n\log\left(\frac{SSW+SSB}{SSW}\right) = n\log\left(1+\frac{SSB}{SSW}\right)$$
...
처리 효과를 나타내는 모수로서 $\tilde\alpha_i=\mu_i-\tilde\mu,\quad \tilde\mu=\frac{1}{k}\sum_{i=1}^k\mu_i$ 로 두고 일원분류정규분포모형을  
$X_{ij} = \tilde\mu + \tilde\alpha_i+e_{ij},\quad \sum_{i=1}^k\tilde\alpha_i=0$ 처럼 쓰기도 한다(가중치 $n_i$가 아닌 단순합 제약).  
이때의 유의성 검정 귀무가설도 결국 $H_0:\mu_1=\cdots=\mu_k$와 동치이므로 동일한 $F$-검정으로 처리한다.
...
TODO:FIXME: 이 부분이 앞에서 안다룬 내용임. 어떤 맥락에서 3절로 따로 나오게 되었는지 생각해보고, 회귀분석이랑 연결지어보기?
...
$Poisson(\delta/2)$ 분포와 $\chi^2(r+2k)$분포의 확률밀도함수를 각각 $pdf_{P(\delta/2)}(k), pdf_{\chi^2(r+2k)}(y)$라 하면 $Y$의 확률밀도함수는 다음과 같다:
...
\therefore\ 2\bigl(l({\hat\theta}^\Omega)-l(\hat\theta^0)\bigr)
...
모수벡터를 $\beta=(\mu,\alpha_1,\dots,\alpha_k)^\top\in\mathbb R^{k+1}$ 로 잡고, 설계행렬을 "상수항 1개 + 집단 더미 $k$개로 잡으면
...
가 된다. 즉, 집단 $i$의 관측치 행들은 반복해서 $(1,\ 0,\dots,0,\ \underbrace{1}_{(i+1)\text{번째 열}},\ 0,\dots,0)$ 꼴이며, 이때 $Y=X\beta+e$ 는 각 관측치에 대해 $X_{ij}=\mu+\alpha_i+e_{ij}$를 정확히 재현한다.

**2) $\mathrm{rank}(X)=k<k+1$ (왜 $X^\top X$가 특이해지는가?)**  

위 $X$는 열이 $k+1$개지만 다음 선형관계가 항상 성립한다: $\text{(상수항 열)}=\sum_{i=1}^k \text{(집단 }i\text{ 더미 열)}$  
