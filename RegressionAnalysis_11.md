...
> 반응함수(response function)란, 설명변수의 값이 주어졌을 때 반응변수 $Y$의 조건부 평균을 나타내는 함수

이제 성별에 따라 반응함수를 구해 보면  
**남자인 경우**: $x_2=0$이므로 $E(y)=\beta_0+\beta_1 x_1$  
**여자인 경우**: $x_2=1$이므로 $E(y)=\beta_0+\beta_1 x_1+\beta_2 =(\beta_0+\beta_2)+\beta_1 x_1$
...
따라서 추정된 회귀식은 $\hat{y}=33.8741-0.1017x_1+8.0555x_2$ 이를 성별에 따라 분리하면 다음과 같다.  
남자: $\hat{y}=33.8741-0.1017x_1$  
여자: $\hat{y}=(33.8741+8.0555)-0.1017x_1 =41.9296-0.1017x_1$

**(1) $\beta_1$** : $\hat{\beta}_1=-0.1017$ 이므로 적성검사 점수가 높을수록 프로그램을 더 빠르게 익힌다고 해석할 수 있다. 예를 들어 점수가 **100점 높아질 경우 평균 소요시간은 약 10시간 감소**한다.  
또한 두 회귀직선의 기울기가 동일하므로 적성검사 점수에 관계없이 남녀 간 평균 차이는 일정하다.

**(2) $\beta_2$** : $\hat{\beta}_2=8.0555$ 이므로 평균적으로 남자가 여자보다 약 **8시간 정도 더 빨리 프로그램을 익힌다**고 해석할 수 있다.
...
앞 절의 모형에서는 두 회귀직선의 기울기가 같다고 가정하였다.
...
교호작용 효과가 존재하는지 검정하는 가설: $H_0:\beta_3=0$, 검정통계량은  $t_0=\frac{\hat{\beta}_3}{\sqrt{\text{Var}(\hat{\beta}_3)}}$.  

여기서 $\text{Var}(\hat{\beta}_3)=c_{33}\sigma^2$ 이고 그 추정값은 $\widehat{\text{Var}(\hat{\beta}_3)} = c_{33}MSE = (0.00003049)(11.02) = 0.000336$  
따라서 $t_0=\frac{-0.0004}{\sqrt{0.000336}}=-0.022$  
유의수준 $\alpha=0.05$에서 $t_{0.025}(16)=2.120$ 이므로 $|t_0|<2.120$  

따라서 **귀무가설 $H_0:\beta_3=0$을 기각할 수 없다.** 즉 이 데이터에서는 **교호작용 효과가 존재하지 않는 것으로 판단된다.**
...
예를 들어 $x_4$를 제거하면 모형은 $y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \beta_3 x_{i3} + \varepsilon_i $이고,  
이때 평균 반응함수(mean response function)는 $E(y)=\beta_0+\beta_1x_1+\beta_2x_2+\beta_3x_3$

각 학력 수준에 대한 반응함수는 다음과 같다.

**고등학교 졸업 ($E_1$)**: 이 경우 $x_2=1, x_3=0$이므로 $E(y)=(\beta_0+\beta_2)+\beta_1x_1$  
**대학교 졸업 ($E_2$)**: 이 경우 $x_2=0, x_3=1$이므로 $E(y)=(\beta_0+\beta_3)+\beta_1x_1$  
...
7장에서는 절단 거듭제곱 기저함수(truncated power basis function)
...
따라서 이론적인 **GLS 추정량** 은 $\hat{\beta}^*=(X^T V^{-1} X)^{-1}X^T V^{-1}y$. 그러나 실제로는 $p_i$가 알려져 있지 않기 때문에 다음과 같은 절차를 사용한다.
...
분산 비교 결과 GLS 추정량의 분산이 더 작다는 것을 확인할 수 있다.  
