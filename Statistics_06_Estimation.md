...
한편 모수 $p$를 이용한 가능도방정식은 

$$\frac 1n \sum_{i=1}^n x_i = p$$
...
따라서 $\mu$에 대해 최대화하면, $l(\mu, \sigma^2)$의 $\mu$계수가 음수이므로,
...
$\eta = g(\theta)$가 일대일이므로, $\theta$와 $\eta$ 사이에 역함수 $\theta = h(\eta)$가 존재한다. 따라서
...
$(X_1, \dots, X_k) \sim Multin(n; p_1, \dots, p_k), \ \sum_{j=1}^k p_j = 1$ 이므로
...
$$\sum_{j=1}^k p_j = \sum_{j=1}^k \frac{x_j}{\lambda} = \frac{n}{\lambda} = 1 \implies \lambda = n$$
...
로, $p_j$는 $0 < p_j < 1$, $\sum_{j=1}^k p_j = 1$, $\sum_{j=1}^k x_j = n$이다. 종속성을 없애기위해 $Y_j = X_j$, $\mu_j = p_j$, $Y=(X_1, \dots, X_k)^\top$, $\mu = (p_1, \dots, p_k)^\top$로 표기하자.  
$\eta_j =\log p_j/p_k, (j=1, \dots, k-1)$로 모수변환을 하면, 다항분포는 다음과 같은 지수족 형태로 표현할 수 있다.

$$ pdf(y; \eta) = \exp\left\{ \sum_{j=1}^{k-1} \eta_j y_j - nA(\eta) + S(y) \right\} $$

여기서 $A(\eta) = \log\left(1 + \sum_{j=1}^{k-1} e^{\eta_j}\right)$, $S(y) = \log\frac{n!}{y_1! \cdots y_k!}$, $x_k = n - \sum_{j=1}^{k-1} x_j$, 모수의 범위는 $-\infty < \eta_j < \infty$이며, 정리 6.3.4의 조건들을 만족한다.  
따라서 모수 $p = (p1, \dots, p_k)^\top$을 이용하면 가능도방정식은

$$E_p(Y_1) = \frac{1}{n}\sum_{i=1}^n y_{i1}, \quad \dots, \quad E_p(Y_{k-1}) = \frac{1}{n}\sum_{i=1}^n y_{i,k-1}$$

$y_{ij}$는 $i$번째 표본의 $j$번째 범주에 속하는 관측값이다. 다항분포에서 $E_p(Y_j) = np_j$이므로, 가능도방정식의 해는

$$\hat p_j = \frac{1}{n}\sum_{i=1}^n y_{ij} = \frac{y_j}{n} = \frac{x_j}{n}$$
...
>**정리:** 쿨백–라이블러 괴리도 $KL(\theta, \theta^0)$는 항상 0 이상이며, 0이 되는 유일한 경우는 $\theta = \theta^0$일 때이다.
...
4. **(기대 로그가능도의 연속성)**
    ...
    $$\forall \theta^0\in\Omega,\quad E_{\theta^0}[\log f(X_1;\theta)]\ \text{가 존재하고 }\theta\ \text{에 관한 연속함수다}$$
...
* 조건 (R2)~(R4)로부터 위 항등식 양변을 $\theta$에 대해 미분하면, $\exp(\log f(x;\theta))=f(x;\theta)$ 이므로
...
\therefore P_\theta\left(|\bar l_n^{(3)}(\theta_n^*)| \gt K\right)
\leq
...
#### 해석
- 근사식 $\sqrt{n}(\hat\theta_n^{\mathrm{MLE}}-\theta)
    = [I(\theta)]^{-1}\sqrt{n}\,\bar{\dot{l}_n}(\theta)+o_n, \quad o_n \xrightarrow{P_\theta} 0$ 이 성립하는것을 알 수 있다.
...
$\Pi^\top(I-\Pi) = 0$이므로 $\Pi(\mathbf{Y} - \mathbf{X}\boldsymbol{\beta})^\top (I - \Pi)\mathbf{Y} = (\mathbf{Y} - \mathbf{X}\boldsymbol{\beta})^\top \Pi^\top (I - \Pi)\mathbf{Y} = 0$
...
오차항 분산의 추정량으로 평균오차제곱(MSE)을 사용한다.
...
$E(\mathbf{e}) = 0$,
$\operatorname{Var}(\mathbf{e}) = \sigma^2 I_n$,
$\operatorname{rank}(\mathbf{X}) = p+1$에서
...
$$\mathbf{Y} - \mathbf{X}\hat{\boldsymbol{\beta}}^{LSE} = \mathbf{Y} - \Pi \mathbf{Y} = (I - \Pi)\mathbf{Y} = (I - \Pi)\mathbf{e}$$

$$\hat{\sigma}^2 = \frac{|\mathbf{Y} - \mathbf{X}\hat{\boldsymbol{\beta}}^{LSE}|^2}{n - p - 1} = \frac{\mathbf{e}^\top (I - \Pi) \mathbf{e}}{n - p - 1}$$

$\mathbf{e}^\top (I - \Pi) \mathbf{e} = \operatorname{trace}\big((I - \Pi) \mathbf{e} \mathbf{e}^\top\big)$, $E[\mathbf{e} \mathbf{e}^\top] = \sigma^2 I_n$ 이므로

$$(n - p - 1) E(\hat\sigma^2) = E\big[\mathbf{e}^\top (I - \Pi) \mathbf{e}\big] = \operatorname{trace}\big((I - \Pi) E[\mathbf{e} \mathbf{e}^\top]\big) = \operatorname{trace}\big((I - \Pi) \sigma^2 I_n\big) \\ = \sigma^2 \operatorname{trace}(I - \Pi)$$

한편 $\operatorname{trace}(A + B) = \operatorname{trace}(A) + \operatorname{trace}(B)$, $\operatorname{trace}(AB) = \operatorname{trace}(BA)$ 이므로

$$\operatorname{trace}(I_n - \mathbf{X}(\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top) = n - \operatorname{trace}(\mathbf{X}(\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top)$$
