...
>### 추가: 실험계획법의 기본원리
>
>실험계획법에는 직교성의 원리, 반복의 원리, 랜덤화의 원리, 블록화의 원리, 교락의 원리와 같은 다섯 가지 기본원리가 있다.
>
>1) 직교성의 원리 (Principle of Orthogonality)
>직교성이란 인자들간의 간섭을 최소화하여 어떤 개의 인자를 동시에 실험에서 사용하더라도, 각각의 주효과 및 교호효과를 모두 분리하여 검증해 낼 수 있는 것을 의미한다. 따라서 인자간의 직교성을 갖도록 실험을 계획하여 데이터를 구하면 같은 실험횟수라도 검증력이 좋은 결과를 확보할 수 있게 되어 직교성은 실험계획의 원리 중 가장 중요한 것으로, 실험의 추를 줄 수 있는 논리적 기반이 될 뿐만 아니라, 인자들의 주효과 및 교호작용의 효과를 모두 검증해 낼 수 있는 실험계획의 기초적인 틀이라 할 수 있다.
>
>2) 반복의 원리 (Principle of Replication)
>여러 집단의 특성지를 비교할 때, 동일 군(Sub-group)으로부터 여러 번의 독립적 무작위 추출을 실시하게 되는데, 이에 같은 여러 번 독립적 무작위 추출이 바로 반복실험의 개념이다. 이때 주의할 점 반드시 동일조건에서 랜덤으로 정해진 숫자만의의 반복실험이 수행되어야 한다는 정이다. 반복은 동일 군(Sub-grou p)내에서의 군내변동을 구하는 유의한 방법이며, 모든 통계적 유의성을 판단하기 위한 오차의 기기를 구할 수 있도록 해준다. 또한 실험의 신뢰도를 높여주는 중요한 역할을 한다. 하지만, 같은 조건에서 두 번 이상 실험한 집들이 비슷한 정도를 보인다면 그 결과치에 대한 신뢰도는 높아질 것이다.
>
>3) 랜덤화의 원리 (Principle of Randomization)
>실험을 수행할 때 아무리 완벽하게 일부된 잡음을 처리한다고 하더라도, 알 수 없는 잡음에 의한 영향을 모두 제거할 수는 없다. 따라서 모든 주효과 및 교호효과를 다양한 잡음의 효과에 무궁 영향을 받게 함으로써 실제도 처리간의 비교에는 잡음에 의한 영향이 서로 상체되도록 할 필요가 있다. 랜덤화 실험에야 알 처리들을 실험단위에 동일한 확률로 배치하는 방법인데, 이를 통해 통계자 어떤 인부잡음에 의한 영향을 각 인자에 균등하게 배분하기 때문에 예방효과를 분석하는 실험의 경우 실험군 과 대조군의 구분을 넓추 둘을 사용하여 실시한다면, 두 실험군이 비티민도 북충여 이어의 다른 요인(체 력의 자이 등)에 대해 평준화되는 효과를 기대할 수 있다.
>
>4) 블록화의 원리 (Principle of Blocking)
>실험환경의 동질성은 민감한 관심의 방법으로는 한계가 있을 수 있다. 특히, 어떤 인부인자가 실험에 영향을 주는 것 알고 있으며, 이에 의한 영향을 제거하기 어려운 경우는 블록화의 방법이 원인이다. 영향력이 큰 실험단위들의 자이를 단순하게 관리에 의해 분군시켰을 경우 오차제곱이 커져 분석력이 떨어질 수 있게 되기 때문에, 동질의 실험단위들을 모아 블록을 만들어 각각의 블록에 모든 처리를 실험하는 것이 합리적 이다 하겠다. 이 블록화 방법은 처리효과에 대한 오염을 막을 수 있게 뿐만 아니라 오차제곱합을 크게 줄여 실험의 정밀도를 높일 수 있다.
>
>5) 교락의 원리 (Principle of Confounding)
>교락이란 두개 이상 인자의 효과가 합쳐져 있어 그 전체효는 구분 수 있으나, 각각의 효과가 열나 되는 자태를 말한다. 일반적으로 교락은 각 효과를 분리해 낼 수 없는 정이기 때문에 효과의 크기가 작으면서 직교성 높이기 위해서는 필요가 없는 상황이나 동일한 원정에서 모든 실험을 완전히 수행할 수 없는 경우 효과의 교호작용이 나타나므로, 효과의 크기가 작다고 상정되는 고자의 교호작용이므로 불필의 효과를 다른 인자효과에 교락시킴으로써 점여이 높은 주효과 교락에 의한 부분요인제실험의 경우 추록 것을 직교계획라고 한다. 다 구하기법은 기초적으로 직교계획에 의한 실험은 근간으로 하기에, 다수의 인자에 의한 실험이 가능해 지는 것이다.
>
