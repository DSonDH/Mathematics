...
### 파스칼 삼각형
파스칼 삼각형은 이항계수 $\binom{n}{k}$를 삼각형 형태로 배열한 것이다. 각 행 $n$은 $0$부터 시작하며, 각 열 $k$도 $0$부터 시작한다.
...
베타 함수 $B(x, y)$는 다음과 같은 적분으로 정의되는 특수 함수.

$$
B(x, y) = \int_0^1 t^{x-1} (1-t)^{y-1} \, dt
$$

여기서 $x$와 $y$는 모두 0보다 큰 값이다. 베타 함수는 감마 함수와 다음과 같은 관계를 가진다.

$$
B(x, y) = \frac{\Gamma(x)\,\Gamma(y)}{\Gamma(x+y)}
$$

베타 함수는 베타 분포, 베이지안 통계 등 다양한 분야에서 활용된다.

## 감마 함수
[자세히 보기](https://namu.wiki/w/%EA%B0%90%EB%A7%88%20%ED%95%A8%EC%88%98)

감마 함수 $\Gamma(z)$는 다음과 같은 적분으로 정의된다.

$$
\Gamma(z) = \int_0^{\infty} e^{-t} t^{z-1} \, dt
$$

여기서 $z$는 0보다 큰 실수 또는 복소수이다. 감마 함수는 팩토리얼 함수의 확장으로, 자연수 $n$에 대해 $\Gamma(n) = (n-1)!$가 성립한다. 감마 함수는 확률론, 통계학, 복소 해석 등 다양한 수학 분야에서 중요한 역할을 한다.

### 스털링의 근사법 (Stirling's approximation)
큰 $n$에 대해 팩토리얼을 근사하는 공식으로, 감마 함수의 점근 전개에서 유도된다.
