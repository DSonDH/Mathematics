...
$$\mathrm{MSE}(\hat\eta,\theta) = E_\theta\left[(\hat\eta(X_1,\dots,X_n)-\eta(\theta))^2\right]$$
...
>#### 조건부 분포 계산
>$\bar X \sim N(\theta, 1/n)$이므로 $\sqrt{n}(\bar X - \theta) \sim N(0,1)$이다.  
>표준정규변수 $Z \sim N(0,1)$을 $Z = \sqrt{n}(\bar X - \theta)$로 정의하면
>
>$$\bar X = \theta + \frac{Z}{\sqrt{n}}$$
>
>$Y = \sum X_i = n\bar X$가 주어졌을 때, $X_1$의 조건부분포는 $Y$를 중심으로 하는 정규분포이다.  
>정규분포의 조건부분포 성질에 의해
>
>$$X_1 \mid Y \sim N\left(\bar X,\, \frac{n-1}{n}\right)$$
>
>즉, $X_1 - \bar X \mid Y \sim N(0, (n-1)/n)$이고, $X_1 - \bar X$와 $\bar X$는 $Y$가 주어졌을 때 조건부독립이다.
>
>따라서
>
>$$P(X_1 > a \mid Y = y) = P\left(\bar X + \sqrt{\frac{n-1}{n}} Z > a \mid \bar X = y\right) = 1 - \Phi\left(\sqrt{\frac{n}{n-1}}(a - y)\right)$$
>
>
>#### MSE 식의 유도
>
...
>
>$$
>E_\theta({\hat\theta^H - \theta})^2 = \theta^2 P_\theta\left(|\bar X| < \frac{1.96}{\sqrt{n}}\right) + E_\theta\left[(\bar X - \theta)^2 I\left(|\bar X| \geq \frac{1.96}{\sqrt{n}}\right)\right]$$

즉, 평균제곱오차는 표준정규확률변수 $Z\sim N(0,1)$를 이용하여
...
- 첫번째 항: 0으로 축소했을 때의 편향 손실: $\theta^2 P_\theta\left(|\bar X| < \frac{1.96}{\sqrt{n}}\right)$ (실제 $\theta \neq 0$일 때 $\hat\theta^H = 0$으로 잘못 추정하는 손실)
- 두번째 항: $\bar{X}$의 분산 손실: $E_\theta\left[(\bar X - \theta)^2 I\left(|\bar X| \geq \frac{1.96}{\sqrt{n}}\right)\right]$ (검정이 기각되어 $\hat\theta^H = \bar X$로 추정하는 경우에만 발생하는 표본평균의 분산 손실)
...
$$ \max_{\theta\in\Omega} E_\theta[(\hat\eta-\eta(\theta))^2]$$
...
$$r(\pi,\hat\eta) =\int_\Omega E_\theta[(\hat\eta-\eta(\theta))^2]\pi(\theta)\,d\theta$$
...
>
>$$E_\theta[\hat\eta^{UE}]^{\theta \in \Omega} \equiv \eta(\theta), \quad \forall \theta \in \Omega$$
>
>와 같이 표기하기도 한다.
...
이는 $\theta$에 의존하지 않는다. 즉, 성공 횟수 $Y$만 알면 순서 정보는 $\theta$ 추론에 불필요하다.
...
$$f(x;\theta_1,\theta_2) = (\theta_2-\theta_1)^{-n} I_{(\theta_1 \le \min X_i,\, \max X_i \le \theta_2)}$$

분해정리에 따라, 함수 $k_1(\min X_i, \max X_i, \theta_1, \theta_2) = (\theta_2-\theta_1)^{-n} I_{(\theta_1 \le \min X_i,\, \max X_i \le \theta_2)}$,와 함수 $k_2(x) = 1$의 곱으로 쓸 수 있으므로, $(\min X_i,\, \max X_i)$이 충분통계량이다.  
즉, 표본의 최솟값과 최댓값만 알면 $\theta_1, \theta_2$에 대한 모든 정보가 보존된다.

### 정리 8.2.4 최대가능도추정량과 충분통계량
MLE $\hat\theta^{\mathrm{MLE}}$가 유일하면, 이는 **임의의 충분통계량 $S=u(X_1, \dots, X_n)$** 의 함수다.
...
라 정의하면 $\hat\eta^{RB} = \hat\eta^{RB}(Y)$는 추정량이고, 아래가 성립한다:

$$\mathrm{MSE}(\hat\eta^{RB}, \theta) = E_{\theta}\left[(\hat\eta^{RB} - \eta(\theta))^2\right] \le \mathrm{MSE}(\hat\eta, \theta) = E_{\theta}\left[(\hat\eta - \eta(\theta))^2\right], \ \forall \theta \in \Omega$$
...
적률이용 추정량: $\hat\theta = 2\bar X$이고, $\hat\theta$는 $\theta$의 불편추정량.  
평균제곱오차(MSE)는

$$
\mathrm{MSE}(\hat\theta, \theta) = E_\theta[(2\bar X - \theta)^2] 
= Var_\theta(2\bar X) + (E_\theta[2\bar X] - \theta)^2
= 4Var_{\theta}(\bar X) = \frac{\theta^2}{3n}
$$

**Rao–Blackwell 개선:**  
Rao–Blackwell 정리에 따라, 충분통계량 $X_{(n)}$에 대한 조건부기대값을 취하면 더 나은 추정량을 얻을 수 있다.

순서통계량을 $X_{(1)} < \dots < X_{(n)}$이라 할 때
...
즉, 정리 8.2.5에서 밝힌바와 같이, Rao–Blackwell 개선을 통해 충분통계량을 사용하면 MSE가 더 작아진다.
...
모집단 분포가 확률밀도함수 $f(x;\theta),\ \theta\in\Omega$중에 하나고, 통계량 $Y = u(X_1, \dots, X_n)$에 대해
...
$$E_\theta[g(Y)] \equiv 0\quad \forall\,\theta\in(0,1)$$

이면 $P_\theta(g(Y)=0)=1$임을 보여야 한다. $E_\theta[g(Y)]$를 전개하면
...
$Y$가 $\theta$에 대한 완비통계량임을 보이려면, 임의의 함수 $g$에 대해 $E_\theta[g(Y)] = 0\quad \forall\,\theta>0$ 이면 $P_\theta(g(Y)=0)=1$임을 보여야 한다.  
$E_\theta[g(Y)]$를 전개하면
...
2. 모수공간$N$이 $k$차원 공간 $R^k$의 열린구간 다면체 $(a_1,b_1)\times \dots \times (a_k,b_k)$를 포함
  - 모수 벡터 η가 충분히 자유롭게, 연속적으로 변화할 수 있는 영역을 가진다
3. $a_1 T_1(x) + \dots + a_k T_k(x)$가 상수 함수가 되려면 $a_1 = \dots = a_k = 0$이어야 한다.

아래는 $\eta = (\eta_1, \dots, \eta_k)^\top \in N$에 관한 **완비충분통계량**이다.
...
즉, $\sum x_i$와 $\sum x_i^2$만 알면 결합확률밀도함수를 모두 표현할 수 있으므로, $Y = (\sum X_i,\, \sum X_i^2)$는 $(\mu, \sigma^2)$에 대한 충분통계량이다.

**2. 완비성 확인**  
정규분포는 지수족이며, $T_1(x) = x$, $T_2(x) = x^2$로 표현된다.  
정리 8.3.2의 조건(모수공간이 열린집합 포함, 비자명한 선형결합이 상수 아님 등)을 모두 만족하므로, $Y = (\sum X_i,\, \sum X_i^2)$는 $(\mu, \sigma^2)$에 대한 완비충분통계량이다.

**3. UMVUE 구하기**  
- $\mu$의 불편추정량: 표본평균 $\bar X = \frac{1}{n}\sum X_i$일때, $E[\bar X] = \mu$이므로 $\bar X$는 $\mu$의 UMVUE이다.
- $\sigma^2$의 불편추정량: $S^2 = \frac{1}{n-1}\sum_{i=1}^n (X_i - \bar X)^2$ 일때, $E[S^2] = \sigma^2$이므로 $S^2$는 $\sigma^2$의 UMVUE이다.
...
**2. 모수공간 확인**  
모수공간 $\mathcal N = \{ (\eta_1, \eta_2): \eta_1 = 1/\mu > 0,\ \eta_2 = -1/(2\mu^2) \}$은 $\eta_2 = -\frac12 \eta_1^2$로 $\eta_1>0$에서 1차원 곡선만을 이룸(2차원 열린집합 아님). 따라서 정리 8.3.2(완비성 보장)를 적용할 수 없다.
...
$$\sum_{y=0}^\infty \delta(y)\frac{e^{-n\theta}(n\theta)^y}{y!} \equiv e^{-2\theta}$$

$$\sum_{y=0}^\infty \frac{\delta(y)}{y!}\lambda^y \equiv e^{(1-2/n)\lambda} \equiv \sum_{y=0}^\infty \frac{((1-2/n)\lambda)^y}{y!}$$
