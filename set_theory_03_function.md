...
즉, 각 $x\in X$에 대해 오직 하나의 $y\in Y$가 대응된다.  
보통 $(x,y)\in f$를 $y=f(x)$로 쓴다.
...
$$
f^{-1} \text{ 존재} 
\Rightarrow \forall y \in Y,\ \exists x \in X \text{ s.t. } (y,x) \in f^{-1} \\
\Rightarrow \forall y \in Y,\ \exists x \in X \text{ s.t. } (x,y) \in f \\
\Rightarrow \forall y \in Y,\ y \in \mathrm{Rng}(f) \\
\Rightarrow Y \subseteq \mathrm{Rng}(f) \\
...
$$
f(x_1) = f(x_2) = y \\
\Rightarrow (x_1, y) \in f \ \land\ (x_2, y) \in f \\
...
- **$g\circ f$가 단사이면 $f$는 단사이다.**  
  ...
  증명: 함수 $f : X \to Y$, $g : Y \to Z$라 하고, 임의의 $x_1, x_2 \in X$에 대하여 $f(x_1) = f(x_2)$ 라 하자.
  
  $$\Rightarrow g(f(x_1)) = g(f(x_2)) \\ \Rightarrow(g \circ f)(x_1) = (g \circ f)(x_2)$$

  그런데 $(g \circ f)$가 단사이므로 $x_1 = x_2$

- **$g\circ f$가 전사이면 $g$는 전사이다.**  

  증명: $(g \circ f)$가 전사라고 하자.  
...
- **정수집합 $\mathbb{Z}$와 자연수집합 $\mathbb{N}$ 사이에는 일대일 대응이 존재한다.**
  ...
  증명: 함수 $f : \mathbb{Z} \to \mathbb{N}$이 다음과 같이 정의되어 있다.
  
  $$ 
  f(x) =
  \begin{cases} 2x + 1, & x \ge 0 \\ -2x, & x < 0 \end{cases} 
  $$
  ...
  **Step1: $f$는 단사임을 증명**  
  ...
  $$
  f(x_1) = f(x_2) \Rightarrow 2x_1 + 1 = 2x_2 + 1 \Rightarrow x_1 = x_2
  $$
...
- ${x}$는 $X$의 원소 $x$를 포함하는 집합이므로, $f(\{x\})$는 $\{f(x)\}$와 같다.
- 증명: $f(\{x\})=\{f(x)\}$는 $f(\{x\})\subseteq \{f(x)\}$와 $\{f(x)\}\subseteq f(\{x\})$가 모두 성립함을 보이는 것과 같다.  
  1. $f(\{x\})\subseteq \{f(x)\}$

  $$
  \forall y \in f(\{x\}),\ \exists x' \in \{x\} \text{ s.t. } y = f(x') \\
  \Rightarrow \forall y \in f(\{x\}),\ y = f(x) \\
  \Rightarrow f(\{x\}) \subseteq \{f(x)\}
  $$

  2. $\{f(x)\}\subseteq f(\{x\})$

  $$
  f(x) \in f(\{x\}) \Rightarrow \{f(x)\} \subseteq f(\{x\})
  $$

3. $f^{-1}(f(A))=A\iff f$는 단사.  
- (⇒) 방향 증명 : ($f^{-1}(f(A)) = A$ 이면 $f$가 단사

    임의의 $x_1,x_2\in X$에 대해 $f(x_1)=f(x_2)$라고 하자. 이때
    ...
    가정에 따라 $f^{-1}(f(\{x_2\})) = \{x_2\}$이므로, $x_1\in\{x_2\}$이다.
    따라서 $x_1=x_2$이고, $f$는 단사이다.
...
4. $f(f^{-1}(B))=B\iff f$는 전사.  
  ...
  $Y = f(f^{-1}(Y)) = Y$이므로 모든 $y \in Y$에 대해 
...
## 부분집합족에 대한 성질
$f:X\to Y$이고 $\{A_\alpha \mid \alpha\in I \}$를 $X$의 부분집합족이면 다음 세가지가 성립한다:
...
>$$
>\begin{aligned}
>y &\in f\Big(\bigcup_{\alpha\in I}A_\alpha\Big) \\
>&\Leftrightarrow \exists x \in \bigcup_{\alpha\in I}A_\alpha \text{ s.t. } y = f(x) \\
>&\Leftrightarrow \exists \alpha \in I,\ x \in A_\alpha \land y = f(x) \\
>&\Leftrightarrow \exists \alpha \in I,\ f(x) \in f(A_\alpha) \\
>&\Leftrightarrow \exists \alpha \in I,\ y \in f(A_\alpha) \\
...
>
>$$
>y \in \bigcap_{\alpha\in I} f(A_\alpha) \Leftrightarrow \forall \alpha \in I,\ y \in f(A_\alpha) \\
>\Leftrightarrow \forall \alpha \in I,\ \exists x_\alpha \in A_\alpha\ \text{s.t. } y = f(x_\alpha)
>$$
>
>단사성이므로
>
...
$$y \in f(A_1) - f(A_2) \Leftrightarrow y \in f(A_1) \land y \notin f(A_2) \\
...
## 추가
임의의 실수함수 $g$에서 $\forall A, B \subseteq \mathbb{R}$,  
