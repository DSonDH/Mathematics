...
즉 오차들이 서로 **독립(independent)**, 분산이 모두 동일 (**등분산성 homoscedasticity**)임을 가정한 것이다. 그러나 실제 데이터에서는 다음과 같은 상황이 빈번하다.
...
OLS 추정량은 $\hat{\beta}=(X^TX)^{-1}X^Ty$ 이었다. 그러나 이제 $\text{Var}(\varepsilon)=\sigma^2 V$ 일 때는 이 추정량이 여전히 **불편(unbiased)** 이지만 **최량선형불편추정량(BLUE: best linear unbiased estimator)** 은 아니다. (정리 3.1 참고)  

따라서 새로운 추정량이 필요하다. 그래서 오차항의 분산관련 행렬을 회귀식 전체에 적용하여 식을 조작한다.
- $T^{−1}$ 으로 모형 전체를 변환한 뒤 OLS를 적용한다.
- == $V^{-1}$로 가중된 잔차제곱합을 최소화한다.
- == 오차의 분산과 상관구조를 고려하여 관측값의 영향력을 조정한다.

**오차 공분산 구조의 분해**

$V$는 대칭이고 양정치 행렬이므로 분해가 가능하다: $C^TVC = A$

* $C$ : 직교행렬 (orthogonal matrix)
* $A$ : $V$의 고유값으로 이루어진 대각행렬

따라서 $V = CAC^T$ 또는 $V=(CA^{1/2})(CA^{1/2})^T$ 여기서 $T = CA^{1/2}$ 라 두면 $V = TT^T$

**선형변환 (Transformation)**

모형 $y = X\beta + \varepsilon$ 양변에 $T^{-1}$을 곱한다: $T^{-1}y = T^{-1}X\beta + T^{-1}\varepsilon$  
여기서 편의를 위해 $\mathbf{z}=T^{-1}y, \quad W=T^{-1}X, \quad \mathbf{v}=T^{-1}\varepsilon$ 라 하면 모형은

$$\mathbf{z} = W\beta + \mathbf{v}$$
...
따라서 변환된 모형은 $\mathbf{z}=W\boldsymbol{\beta}+\mathbf{v}, \quad \mathbf{v}\sim N(\mathbf{0},\sigma^2 I_n)$, 즉 **표준 선형회귀모형**이 된다.
...
= \left[X^T(T^T)^{-1}T^{-1}X\right]^{-1} X^T(T^T)^{-1}T^{-1}\mathbf{y} \\
=(X^TV^{-1}X)^{-1}X^TV^{-1}\mathbf{y}$$
...
GLS 추정량 $\hat{\boldsymbol{\beta}}^*=(X^TV^{-1}X)^{-1}X^TV^{-1}\mathbf{y}$ 의 성질을 분석해보자
 - 대부분 기존 식들에 $X^TX$ 사이에 $V^{-1}$ 끼워진 형태

### 기대값 (Expectation)

$$E(\hat{\boldsymbol{\beta}}^*) = (X^TV^{-1}X)^{-1}X^TV^{-1}E(\mathbf{y}) = (X^TV^{-1}X)^{-1}X^TV^{-1}X\boldsymbol{\beta} = \boldsymbol{\beta}$$

즉, GLS 추정량은 **불편추정량(unbiased estimator)** 이다.

### 분산 (Variance)

$\text{Var}(\mathbf{y})=\sigma^2V$ 이므로

$$
\begin{aligned}
\text{Var}(\hat{\boldsymbol{\beta}}^*) &=\text{Var}[(X^TV^{-1}X)^{-1}X^TV^{-1}\mathbf{y}]\\
&=(X^TV^{-1}X)^{-1}X^TV^{-1}\text{Var}(\mathbf{y})V^{-1}X(X^TV^{-1}X)^{-1}\\
&=\sigma^2 (X^TV^{-1}X)^{-1} \\
\end{aligned}
$$
...
\text{Var}(\mathbf{e}^*) = [I_n - X(X^\top V^{-1}X)^{-1}X^\top V^{-1}](\sigma^2 V) \\
...
$$
\begin{aligned}
SSE(\hat{\boldsymbol{\beta}}^*) &= (\mathbf{z} - W\hat{\boldsymbol{\beta}}^*)^T (\mathbf{z} - W\hat{\boldsymbol{\beta}}^*)\\
&= (\mathbf{y}-X\hat{\boldsymbol{\beta}}^*)^TV^{-1}(\mathbf{y}-X\hat{\boldsymbol{\beta}}^*) \\
&= (\mathbf{e}^*)^TV^{-1}(\mathbf{e}^*) \\
&= \mathbf{y}V^{-1}\left[I_n - X(X^\top V^{-1}X)^{-1}X^\top V^{-1}\right]\mathbf{y}
\end{aligned}
$$

### SSE의 기대값
정리 3.1과  잔차벡터 식을 이용하면 ($V$는 $n \times n$ 정칙행렬)

$$E[SSE(\hat{\boldsymbol{\beta}}^*)] = E[(\mathbf{e}^*)^TV^{-1}(\mathbf{e}^*)] = \text{tr}[V^{-1}\text{Var}(\mathbf{e}^*)] \\
= \text{tr}[V^{-1}(\sigma^2V - \sigma^2X(X^\top V^{-1}X)^{-1}X^\top)] \\
= \sigma^2 \text{tr}[I_n - V^{-1}X(X^\top V^{-1}X)^{-1}X^\top] \\
= \sigma^2 (\text{tr}[I_n] -\text{tr}[I_q]) \\
\sigma^2 (n-p-1)$$
...
### 회귀계수 분산의 추정
$\text{Var}(\hat{\boldsymbol{\beta}}^*)$의 불편추정량은 $\text{Var}(\hat{\boldsymbol{\beta}}^*)=\sigma^2(X^TV^{-1}X)^{-1}$ 이므로

$$\widehat{\text{Var}}(\hat{\boldsymbol{\beta}}^*)=\tilde{\sigma}^2 (X^TV^{-1}X)^{-1}$$

### OLS 분산추정량의 문제

만약 OLS에서 사용한 

$$\tilde{\sigma}^2=MSE = \frac{(\mathbf{y}-X\hat{\boldsymbol{\beta}})^T(\mathbf{y}-X\hat{\boldsymbol{\beta}})}{(n-p-1)} = \frac{\mathbf{y}\left[I_n - X(X^\top X)^{-1}X^\top \right]\mathbf{y}}{n-p-1}$$

를 그대로 사용하면 $\hat{\sigma}^2 (X^TV^{-1}X)^{-1}$ 은 $\text{Var}(\hat{\boldsymbol{\beta}}^*)$의 불편추정량이 아니다. 왜냐하면 정리3.1을 사용하면

$$
E(\hat{\sigma}^2) = \frac1{n-p-1}E[\mathbf{y}\left[I_n - X(X^\top X)^{-1}X^\top \right]\mathbf{y}] \\
= \frac{\sigma^2}{n-p-1}[\text{tr}(V)-\text{tr}(X(X^\top X)^{-1}X^\top V)]
\neq \sigma^2
...
- 모든 관측값 사이에 동일한 상관을 부여하는 $\rho J_n$ 부분이 절편 방향인 $\mathbf1_n$ 방향에만 작용한다는 것이다. OLS 잔차는 절편 방향과 직교하므로 이 공통상관 구조가 회귀계수 추정값을 바꾸지 않는다.
- 다만 절편항이 없다면 일반적으로 위 결론은 성립하지 않는다.
...
이제 $g$개의 방정식을 하나로 묶는다. 각 방정식이 $\mathbf{y}_h = X\boldsymbol{\beta}_h + \boldsymbol{\varepsilon}_h, \quad h=1, \dots, g$ 로 주어졌으므로, 이들을 세로로 쌓아 하나의 큰 벡터식으로 만들 수 있다. 
...
$$\text{Var}(\boldsymbol{\varepsilon})=\Sigma\otimes I_n=\Omega
\tag{10.19}$$
...
따라서 $\Sigma\otimes I_n$는 $\Sigma$의 각 원소 $\sigma_{hq}$를 $\sigma_{hq}I_n$라는 블록으로 바꾼 행렬이다. 위에서 유도한 $\text{Var}(\boldsymbol{\varepsilon})$와 정확히 같은 구조가 된다.
...
전체 시스템은 $\mathbf{y}=Z\boldsymbol{\beta}+\boldsymbol{\varepsilon}$ 인데 

$$\text{Var}(\boldsymbol{\varepsilon})=\Omega=\Sigma\otimes I_n$$
...
이제 매우 중요한 결과가 나온다. 식 (10.14)의 다변량 회귀에서 **오차에 대한 가정 (10.15), (10.16)이 성립**하고, **모든 방정식의 계획행렬이 동일하게 $X$** 인 경우에는

$$\hat{\boldsymbol{\beta}}^{*}=\hat{\boldsymbol{\beta}}$$

가 성립한다. 즉, GLS를 쓰든 각 방정식을 따로 OLS로 추정하든 결과가 같다.

이것은 직관적으로는 다소 놀라운 결과이다. 오차들 사이에 동시공분산이 있으므로 보통은 GLS가 OLS보다 더 효율적일 것 같지만, **모든 방정식의 설명변수 행렬이 완전히 동일하고 오차항 가정 두가지를 만족하면** 그 상관구조를 이용해도 계수 추정 자체는 달라지지 않는다.  
...
>
>**1. $(Z^T\Omega^{-1}Z)^{-1}$ 계산**  
>먼저 $Z^T\Omega^{-1} = (I_g\otimes X)^T(\Sigma^{-1}\otimes I_n)$에서, $(I_g\otimes X)^T = I_g\otimes X^T$ 이므로
>
...
>
>한편 $Z^T\Omega^{-1}\mathbf{y} =(\Sigma^{-1}\otimes X^T)\mathbf{y}$ 이므로 GLS 추정량은
>
...
> (참고) 이것이 바로 **겉보기에는 서로 무관해 보이는 회귀식들이 오차 상관 때문에 사실상 정보를 공유한다**는 점이다. 방정식별 설명변수가 다르면, 다른 방정식의 정보가 해당 방정식의 추정 정밀도를 개선할 수 있다. 이것이 SUR(Seemingly Unrelated Regression)류 모형의 핵심 직관이다.

### (참고) $\Omega$가 알려져 있지 않을 때의 처리

현실에서는 $\Omega=\Sigma\otimes I_n$를 직접 알 수 없는 경우가 거의 대부분이다. 특히 $\Sigma$의 원소들 $\sigma_{hq}$를 모르는 경우가 많다. 이때는 먼저 $\Sigma$를 추정한 뒤, 그 추정값을 GLS 공식에 대입하는 방법을 사용한다. 
...
로 계산한다. 이제 $\Sigma$의 추정량을 $S=(S_{hq})$로 두고, 각 원소를

$$S_{hq}=\frac{\mathbf{e}_h^T \mathbf{e}_q}{n-p_h-1} \tag{10.25}$$

로 정의한다. 여기서 $p_h+1$은 $X_h$의 열의 개수이므로, $p_h$는 절편을 제외한 설명변수의 수로 볼 수 있다. 분모 $n-p_h-1$은 해당 방정식의 자유도(degree of freedom)이다.

이렇게 얻은 $S$를 이용하면 $\Omega$의 추정량을 $S\otimes I$ 로 두고, 실행가능 GLS 추정량을
...
자기상관(autocorrelation)은 회귀모형의 오차항(error term)들이 서로 독립이 아니라 **시차(lag)를 두고 상관관계**를 갖는 현상을 말한다. 특히 시계열자료(time series data)에서는 인접한 시점의 오차가 서로 연관되는 경우가 매우 흔하다.  
...
$$\delta_i \sim N(0,\sigma_\delta^2),\quad \text{Cov}(\delta_i,\delta_j)=0,\quad i\neq j$$
...
실제로 자기상관계수 $\rho$를 모르는 경우가 많다. 이때는 $\rho$를 먼저 추정하고, 그 추정값으로 $V$를 추정한 뒤 GLS를 수행한다. 하지만 이 방법은 식(10.29)는 BLUE가 아니게된다.  

자기상관계수를 추정하는 방법으로, 단순회귀모형의 $\beta$를 OLS로 먼저 추정하여 잔차(residual)를 구한 뒤, 그 잔차를 이용하여 $\rho$를 추정하는 방법이 있다. 잔차 $e_i$를 사용하고 28번식의 가정으로부터 OLS추정법으로 $\rho$를 추정한다.

$$\hat{\rho} = \frac{\sum_{i=2}^n (e_{i-1} - \bar{e}_{1})(e_i - \bar{e}_2)}{\sum_{i=2}^n (e_{i-1} - \bar{e}_{1})^2} \tag{10.30} \\
\bar{e}_1=\frac{1}{n-1}\sum_{i=2}^n e_{i-1},\qquad \bar{e}_{2}=\frac{1}{n-1}\sum_{i=2}^n e_{i}$$

($\bar{e}_1$와 $\bar{e}_2$는 거의 0에 가까우므로 보통 0으로 놓고 계산한다)
...
$$\hat{\boldsymbol{\beta}}^*=[X^T\hat{V}^{-1}X]^{-1}X^T\hat{V}^{-1}\mathbf{y} \tag{10.31}$$
...
$$
\begin{aligned}
\text{Var}(\varepsilon_i') &= \text{Var}(\varepsilon_i-\rho\varepsilon_{i-1}) \\
&= \text{Var}(\varepsilon_i) + \rho^2 \text{Var}(\varepsilon_{i-1}) - 2\rho \text{Cov}(\varepsilon_i,\varepsilon_{i-1})\\
...
&= E[\varepsilon_i\varepsilon_{i-1} -\rho \varepsilon_i\varepsilon_{i-2} -\rho \varepsilon_{i-1}^2 +\rho^2 \varepsilon_{i-1}\varepsilon_{i-2}]
\end{aligned}
$$
...
으로, 식 (10.36)의 $\beta_0'$와 $\beta_1$의 BLUE는 OLS 추정법으로 얻어진다.  
$\beta_0'=\beta_0(1-\rho)$ 관계를 이용하여 $\beta_0$를 복원한다.

하지만, 자기상관계수$\rho$가 미지이면 식 35번을 얻을 수 없으므로 데이터 변환방법을 사용할 수 없다. 따라서 데이터에서 추정값을 얻어야 한다: 다음과 같은 추정량을 제시한다.
...
엄밀히 말하면 변환은 보통 $i=2,\dots,n$부터 유효하며, 첫 관측치는 별도 처리 또는 소거하는 경우가 많다.
...
어떤 큰 회사의 **연구개발비**와 **총판매액**에 대한 분기별 데이터를 이용하여 다음 단순회귀모형을 적합하려고 한다. $y_i=\beta_0+\beta_1x_i+\varepsilon_i,\qquad i=1,2,\cdots,20$

* $y_i$: 총판매액(억원)
* $x_i$: 연구개발비(백만원)

먼저 **일차자기상관계수 $\rho=0$인지 검정**하라고 한다. 경제 자료에서는 $\rho$가 대체로 양(positive)인 경우가 많으므로 다음과 같은 가설을 둔다: $H_0:\rho=0, \quad H_1:\rho>0$
...
변환된 데이터에 대해 식 (10.39)의 OLS 추정량을 적용하면 $\hat{\beta}_0 = \frac{\hat{\beta}_0'}{1-\hat{\rho}} = \frac{40.2457}{1-0.891} \approx 369.2266, \
\hat{\beta}_1=0.4862$

즉 자기상관을 무시하고 원자료에 OLS를 적용했을 때의 추정치 $\hat{\beta}_0=89.2339,\quad \hat{\beta}_1=2.0242$ 와,  
자기상관을 제거한 뒤의 GLS형 추정치 $\hat{\beta}_0=369.2266,\quad \hat{\beta}_1=0.4862$ 사이에 큰 차이가 나타난다.  
앞의 추정치는 자기상관을 무시한 단순 OLS 결과이고, 뒤의 추정치는 오차 간 자기상관을 제거한 뒤 OLS를 적용한 것이므로 보다 적절한 GLS 추정 결과이다.
