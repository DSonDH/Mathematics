...
이 경우 단순선형회귀(simple linear regression)는 적절하지 않으며 **다항항(polynomial term)** 을 포함한 회귀모형을 고려해야 한다.
...
### 7.1.3 행렬 표현 (Matrix Formulation)
데이터가 $n$개 있을 때
...
### 7.1.4 최소제곱추정 (Least Squares Estimation)
회귀계수는 최소제곱법(least squares method)으로 추정한다.
...
### 산점도를 이용한 판단
데이터 $(x_i, y_i)$의 산점도를 먼저 그려 보면 곡선 형태를 어느 정도 추정할 수 있다.
...
회귀모형을 적합한 후 **잔차(residuals)** 를 분석한다.
...
즉
* 잔차에 구조가 보이면 모형이 부적절하다.

### 순차 F-검정 (Sequential F-test)
차수를 하나씩 증가시키며 검정을 수행한다.
절차는 다음과 같다.
...
따라서
* **3차 이상 모형은 거의 사용되지 않는다.**

**항 선택 문제 (Term Selection)**  
이차모형이 적합하다고 판단되어도 **모든 항을 반드시 포함할 필요는 없다.**

예를 들어
$$y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_{11}x_1^2 + \beta_{22}x_2^2 + \beta_{12}x_1x_2$$
에서

부분 F-검정(partial F-test)을 통해
$$H_0 : \beta_{12}=0$$
을 기각하지 못하면
...
**변수 선택 문제**  
어떤 항을 포함할지 결정하는 문제는
**변수 선택 (Selection of Variables)** 문제이다.
대표적인 방법은 다음과 같다.
* 부분 F-검정
...
라 하고 수준 간격을 $d$라 하면 다음과 같은 직교다항식이 만들어진다.
...
| 1차  | $SS(\hat{\beta_1})$ | 1       | $MS(\hat{\beta_1})$ | $MS(\hat{\beta_1})/MSE$ |
| 2차  | $SS(\hat{\beta_2})$ | 1       | $MS(\hat{\beta_2})$ | $MS(\hat{\beta_2})/MSE$ |
| ... | ...           | ...     | ...           | ...               |
| k차  | $SS(\hat{\beta_k})$ | 1       | $MS(\hat{\beta_k})$ | $MS(\hat{\beta_k})/MSE$ |
| 잔차  | SSE           | $n-k-1$ | MSE           |                   |
| 전체  | SST           | $n-1$   |               |                   |
...
\hat{\beta_2}=-0.719$$
따라서
$$\hat{y} = 96.4 + 3.621 p_1(x) - 0.719 p_2(x)$$
...
* 2011–2015 : 완만한 증가
* 2015 이후 : 급격한 증가
따라서 **2015년을 경계점**으로 모형을 나누는 것이 적절하다.
...
**미분 연속 조건**  
만약 $\frac{d\eta(x)}{dx}$도 연속이면
$$2a(\beta_{22}-\beta_{12}) + (\beta_{21}-\beta_{11}) = 0$$
이 되어$\beta_4 = 0$이 된다.
...
모형
$$\eta(x)=\beta_0 + \beta_1x + \beta_2T_1(x-4)$$
...
지금까지 우리는
$$(x_i,y_i),\ i=1,\ldots,n$$
의 데이터가 **이미 존재한다고 가정**하였다.

그러나 실제 연구에서는
* 어떤 $x$ 값에서
* 몇 번 측정할 것인지

를 **실험계획 (design of experiments)** 을 통해 결정할 수 있다.
설명변수의 관심 영역을 $a \le x \le b$ 라고 하자. 데이터를 $n$개 측정한다고 해도 반드시 **서로 다른 $n$개의 $x$** 를 사용할 필요는 없다.  
예를 들어 $k < n$개의 수준을 선택하고 각 수준에서 $n_1,n_2,\ldots,n_k$번 측정하여 $n=n_1+n_2+\cdots+n_k$이 되도록 할 수도 있다.
...
분산행렬의 행렬식
$$|Var(\hat{\beta})| = |\sigma^2 (X^TX)^{-1}|$$
을 **일반화분산 (generalized variance)** 이라고 한다.
...
분산행렬의 대각합
$$tr(\sigma^2(X^TX)^{-1})$$
을 최소화하는 계획을 **A-최적계획 (A-optimal design)** 이라고 한다.

이는
$$Var(\hat{\beta_0})+\cdots+Var(\hat{\beta_p})$$
의 합을 최소화한다. D-최적계획과 A-최적계획은 **항상 일치하지 않는다.**
