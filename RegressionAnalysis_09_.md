# Chapter 9 변수의 선택 (Variable Selection)

이 장에서는 반응변수 $y$와 이 변수에 영향을 미칠 수 있는 가능한 모든 설명변수 $x_1, x_2, \dots, x_p$가 주어졌다고 가정한다. 일반적으로 반응변수 $y$의 변화를 설명하기 위한 모형을 찾을 때, 모든 설명변수를 전부 포함한 모형보다는 **가능한 적은 수의 변수를 사용한 간단한 모형**을 선택하는 것이 바람직한 경우가 많다. 이러한 모형을 간명모형 (parsimonious model)이라고 한다.

이 장의 핵심 문제는 다음과 같다.
회귀모형 (regression model)에서 설명변수들을 어떻게 선택할 것인가?

첫째, $y$에 영향을 미칠 수 있는 모든 설명변수 $x$를 사용하여 $y$의 값을 예측해야 한다. 그렇지 않으면 $y$의 편의가 증가할 수 있다. 여기서 편의는 편향 (bias)을 뜻한다.

둘째, 설명변수가 많을수록 데이터를 수집하고 분석하며 변수들의 변화를 탐지하거나 관리하는 데 큰 노력이 요구될 뿐 아니라, $\hat{y}$의 분산이 증가할 수 있으므로 가능한 범위 내에서 적은 수의 설명변수를 포함해야 한다.

즉, 회귀분석에서 설명변수 선택은 아래 두 요구 사이의 균형 문제이다.

* 중요한 설명변수를 빠뜨리지 말아야 한다.
* 그러나 설명변수를 너무 많이 넣어 분산을 키우거나 모형을 복잡하게 만들어서도 안 된다.

이 두 원칙이 **서로 이율배반적**이므로, 적절한 판정기준 (criterion)을 토대로 설명변수를 선택함으로써 '최적' 회귀모형 (best regression model)을 선택해야 한다고 설명한다. 여기서 '최적'이라는 표현은 절대적 의미가 아니라 **판정기준에 따라 달라지는 상대적 개념**이다. 즉, 판정기준이 달라지면 최적회귀모형도 달라질 수 있다.

그리고 본격적으로 변수 선택 방법을 논하기 전에, **모든 설명변수를 포함시키지 않고 일부만 선택함으로써 발생하는 회귀분석의 이론적 결과**를 먼저 다룬다. 이것이 곧 9.1 절의 내용이다.

## 9.1 변수제거의 결과 (Consequences of Variable Deletion)

이 절은 설명변수 일부를 제거하여 축소모형 (reduced model)을 사용할 때, 회귀계수 추정과 예측에 어떤 변화가 생기는지를 이론적으로 정리한 부분이다. 핵심 주제는 다음 세 가지이다.

1. 변수를 제거하면 추정량에 **편향 (bias)** 이 생길 수 있다.
2. 그러나 동시에 추정량 또는 예측량의 **분산 (variance)** 이 줄어들 수 있다.
3. 따라서 전체 성능은 평균제곱오차 (mean squared error, MSE)의 관점에서 판단해야 한다.

### 완전모형 (Full Model)의 설정

설명변수가 총 $p$개 있고, 자료는 $n$개 있으며, $n > p$라고 하자. 중회귀모형 (multiple regression model)을 다음과 같이 둔다. 이 중회귀모형을 **완전모형 (full model)** 이라고 부른다.

$$y_i = \beta_0 + \beta_1 x_{i1} + \cdots + \beta_p x_{ip} + \epsilon_i, \qquad \epsilon_i \sim N(0,\sigma^2), \quad i=1,2,\dots,n \tag{11.1}$$

이를 행렬 형태로 쓰면

$$\mathbf{y} = X\beta + \epsilon, \qquad \epsilon \sim N(0,\sigma^2 I_n) \tag{11.2}$$

* $X$는 $n \times (p+1)$ 행렬
* $\beta$는 $(p+1)\times 1$ 벡터
* $\mathbf{y}$와 $\epsilon$은 $n \times 1$ 벡터

즉, 완전모형은 가능한 모든 설명변수를 포함한 모형

### 설명변수의 분할

이제 전체 설명변수 집합을 두 부분으로 나눈다. 행렬 $X$를

$$X = (X_k, X_r)$$

로 나눈다고 하자. 그러면 모형을 다시

$$\mathbf{y} = X_k \beta_k + X_r \beta_r + \epsilon \tag{11.3}$$

으로 표현할 수 있다.

여기서

* $X_k$는 절편 (intercept)과 $k$개의 설명변수를 포함한다.
* $X_r$는 나머지 $p-k$개의 설명변수를 포함한다.
* $\beta_k$와 $\beta_r$는 각각 $X_k$, $X_r$에 대응되는 회귀계수 벡터이다.

즉, $X_k$는 "남길 변수들", $X_r$는 "제거될 변수들"이라고 생각하면 된다.

### 완전모형에서의 최소제곱추정량 (Least Squares Estimator)

완전모형에서 $\beta$의 최소제곱추정량 (least squares estimator)을 $\hat{\beta}$라고 하자. 이를 분할하여

$$\hat{\beta}
=
\begin{bmatrix}
\hat{\beta}_k \\
\hat{\beta}_r
\end{bmatrix}
=
(X^TX)^{-1}X^T\mathbf{y} \tag{11.4}$$

라고 쓴다.

즉, 완전모형에서는 설명변수 전체를 사용하여 $\beta_k$와 $\beta_r$를 동시에 추정한다.

### 축소모형 (Reduced Model)의 최소제곱추정량

이제 식 (11.3)에서 $p-k$개의 설명변수를 누락시키고, $k$개의 설명변수만으로 모형을 적합한다고 하자. 그러면 축소모형은

$$\mathbf{y} = X_k \beta_k + \epsilon$$

의 형태가 된다. 이 축소모형에 대한 최소제곱추정량은

$$\tilde{\beta}_k = (X_k^T X_k)^{-1} X_k^T \mathbf{y} \tag{11.5}$$

이다.

중요한 점은 $\tilde{\beta}_k$가 단순히 $\hat{\beta}_k$와 같은 것이 아니라는 점이다.
$\hat{\beta}_k$는 전체 변수를 넣은 상태에서 얻은 추정량의 일부이고, $\tilde{\beta}_k$는 실제로 나머지 변수를 제거한 채로 다시 적합한 추정량이다. 이 둘은 일반적으로 다르다.

### 완전모형과 축소모형의 오차분산 추정량

$\hat{\sigma}^2$와 $\tilde{\sigma}^2$를 각각 완전모형과 축소모형의 잔차평균제곱 (residual mean square)이라고 두고 다음과 같이 쓴다.

완전모형에서는

$$\hat{\sigma}^2
=
\mathbf{y}^T
\left[
I_n - X(X^TX)^{-1}X^T
\right]
\mathbf{y}
\big/
(n-p-1) \tag{11.6}$$

축소모형에서는

$$\tilde{\sigma}^2
=
\mathbf{y}^T
\left[
I_n - X_k(X_k^TX_k)^{-1}X_k^T
\right]
\mathbf{y}
\big/
(n-k-1) \tag{11.7}$$

이다.

여기서 분모의 자유도 (degrees of freedom)가 달라지는 점에 유의해야 한다.

* 완전모형 자유도: $n-p-1$
* 축소모형 자유도: $n-k-1$

즉, 축소모형은 더 적은 수의 변수를 쓰므로 자유도가 더 커진다.

### 축소모형 추정량의 기대값: 편향의 발생

이제 $\tilde{\beta}_k$의 기대값을 계산한다.

$$E(\tilde{\beta}_k)=E\left[(X_k^TX_k)^{-1}X_k^T\mathbf{y}\right]$$

$$=(X_k^TX_k)^{-1}X_k^T E(\mathbf{y})$$

완전모형에서 $E(\mathbf{y}) = X\beta$이므로,

$$=(X_k^TX_k)^{-1}X_k^T X\beta$$

그리고 $X=(X_k,X_r)$, $\beta = \begin{bmatrix}\beta_k \\ \beta_r\end{bmatrix}$ 이므로

$$=(X_k^TX_k)^{-1}X_k^T [X_k\beta_k + X_r\beta_r]$$

$$=\beta_k + (X_k^TX_k)^{-1}X_k^T X_r \beta_r$$

여기서 $A = (X_k^TX_k)^{-1}X_k^T X_r$로 놓으면

$$E(\tilde{\beta}_k) = \beta_k + A\beta_r \tag{11.8}$$

을 얻는다.

이 식의 의미는 매우 중요하다. $\tilde{\beta}_k$의 기대값이 $\beta_k$와 정확히 일치하지 않고, 추가항 $A\beta_r$가 붙는다. 즉, 일반적으로 축소모형의 추정량 $\tilde{\beta}_k$는 **불편추정량 (unbiased estimator)** 이 아니다. 설명변수 제거로 인해 **편향 (bias)** 이 생긴다.

이 편향은 구체적으로

$$\text{Bias}(\tilde{\beta}_k)=E(\tilde{\beta}_k)-\beta_k=A\beta_r$$

라고 쓸 수 있다.

즉, 제거한 변수들의 계수 $\beta_r$이 0이 아니고, 또한 제거된 변수 $X_r$이 남겨진 변수 $X_k$와 상관되어 있으면 $A\beta_r\neq 0$가 되어 편향이 발생한다.

이는 회귀분석에서 잘 알려진 **누락변수편향 (omitted variable bias)** 의 행렬형 표현이라고 볼 수 있다.
식 (11.8)은 사실상 누락변수편향의 핵심 수식을 제시한 것이다.

### 축소모형 추정량의 분산

다음으로 $\tilde{\beta}_k$의 분산은

$$\text{Var}(\tilde{\beta}_k)=\text{Var}[(X_k^TX_k)^{-1}X_k^T\mathbf{y}]$$

$$(X_k^TX_k)^{-1}X_k^T \, \text{Var}(\mathbf{y}) \, X_k (X_k^TX_k)^{-1}$$

완전모형에서 $\text{Var}(\mathbf{y}) = \sigma^2 I_n$ 이므로

$$=\sigma^2 (X_k^TX_k)^{-1}X_k^T I_n X_k (X_k^TX_k)^{-1}$$

$$=\sigma^2 (X_k^TX_k)^{-1} \tag{11.9}$$

이다.

이 결과는 형식상 일반적인 최소제곱추정량의 분산 공식을 따른다. 다만 중요한 것은 이것이 **편향이 있는 추정량의 분산**이라는 점이다. 즉, 분산만 보고 좋다고 판단하면 안 되고, 편향까지 함께 봐야 한다.

### 평균제곱오차 (Mean Squared Error, MSE)

다음으로 $(\tilde{\beta}_k-\beta_k)(\tilde{\beta}_k-\beta_k)^T$의 기대값을 $\tilde{\beta}_k$의 평균제곱오차 (mean squared error, MSE)라고 정의한다. 즉,

$$\text{MSE}(\tilde{\beta}_k)=E[(\tilde{\beta}_k-\beta_k)(\tilde{\beta}_k-\beta_k)^T]$$

이다.

이를 편향-분산 분해 (bias-variance decomposition)하면

$$\text{MSE}(\tilde{\beta}_k)=\text{Var}(\tilde{\beta}_k)+[E(\tilde{\beta}_k)-\beta_k][E(\tilde{\beta}_k)-\beta_k]^T$$

이고, 식 (11.8), (11.9)를 이용하면

$$\text{MSE}(\tilde{\beta}_k)=\sigma^2(X_k^T X_k)^{-1} + A\beta_r\beta_r^T A^T \tag{11.10}$$

이 된다.

이 식은 변수 제거 문제의 핵심을 정확히 보여 준다.

* 첫 번째 항 $\sigma^2(X_k^TX_k)^{-1}$: 분산 (variance) 부분
* 두 번째 항 $A\beta_r\beta_r^T A^T$: 편향 제곱 (squared bias) 부분

즉, 변수를 제거하면 분산은 줄어들 수 있지만, 그 대가로 편향이 생긴다. 평균제곱오차는 두 효과를 함께 평가한다.

### 축소모형 오차분산 추정량의 기대값

이어서 $\tilde{\sigma}^2$의 기대값을 다음과 같이 제시한다.

$$E(\tilde{\sigma}^2)=\sigma^2+\frac{\beta_r^T X_r^T\left[I - X_k(X_k^TX_k)^{-1}X_k^T\right]X_r \beta_r}{n-k-1} \tag{11.11}$$

즉, 축소모형에서의 잔차평균제곱은 일반적으로 $\sigma^2$의 불편추정량이 아니다. 제거된 변수들이 실제로 유의한 역할을 하고 있다면, 그 효과가 잔차로 밀려 들어가므로 $\tilde{\sigma}^2$는 실제 오차분산보다 커질 수 있다.

여기서 MSE는 평균제곱오차 (mean squared error)를 뜻하는 것이며, 분산분석표 (ANOVA table) 등에서 사용되는 잔차평균제곱 (MSE)와는 다른 맥락이다. 따라서 평균제곱오차를 쓸 때는 $\text{MSE}(\cdot)$처럼 괄호를 붙여 써야한다.

### 예측 문제로의 전개

이제 설명변수 $(x_1,\dots,x_p)$가 주어졌을 때 $E(y)$를 예측하는 문제를 생각한다. 완전모형에서

$$E(y) = \beta_0 + \beta_1 x_1 + \cdots + \beta_p x_p = x^T\beta$$

여기서 $x$를 $x_k$와 $x_r$로 나누면

$$E(y) = x_k^T \beta_k + x_r^T \beta_r$$

그리고 $E(y)$의 추정량은 완전모형과 축소모형에서 각각 다음과 같다.

완전모형에서는

$$\hat{y} = x^T\hat{\beta} = x_k^T \hat{\beta}_k + x_r^T \hat{\beta}_r \tag{11.12}$$

축소모형에서는

$$\tilde{y}_k = x_k^T \tilde{\beta}_k$$

이다.

### 완전모형 예측량과 축소모형 예측량의 기대값, 분산

완전모형 예측량에 대해서

$$E(\hat{y}) = x^T\beta$$

$$\text{Var}(\hat{y}) = \sigma^2 x^T (X^TX)^{-1} x \tag{11.13}$$

이다.

즉, 완전모형 예측량 $\hat{y}$는 불편하다.

반면 축소모형 예측량은

$$E(\tilde{y}_k) = x_k^T\beta_k + x_k^T A\beta_r$$

$$\text{Var}(\tilde{y}_k) = \sigma^2 x_k^T (X_k^TX_k)^{-1} x_k$$

이다.

여기서 $E(y)=x_k^T\beta_k + x_r^T\beta_r$이므로, 일반적으로

$$E(\tilde{y}_k)\neq E(y)$$

이다. 즉, 축소모형 예측량 역시 편향을 가진다.

### 축소모형 예측의 평균제곱오차

축소모형에서 $E(y)$의 예측값 $\tilde{y}_k$에 대한 평균제곱오차는

$$\text{MSE}(\tilde{y}_k)=E(\tilde{y}_k - x^T\beta)^2 \tag{11.14}$$

로 정의된다. 이를 분해하면

$$\text{MSE}(\tilde{y}_k)=E(\tilde{y}_k - E(\tilde{y}_k))^2+[E(\tilde{y}_k)-x^T\beta]^2$$

즉,

$$\text{MSE}(\tilde{y}_k)=\text{Var}(\tilde{y}_k)+[(x_k^T\beta_k + x_k^T A\beta_r) - (x_k^T\beta_k + x_r^T\beta_r)]^2$$

따라서

$$\text{MSE}(\tilde{y}_k)=\sigma^2 x_k^T (X_k^TX_k)^{-1} x_k+(x_k^T A\beta_r - x_r^T\beta_r)^2$$

가 된다.

이 역시 분산항 + 편향제곱항의 구조이다. 즉, 예측에서도 본질은 동일하다. 변수를 제거하면 예측 분산은 줄어들 수 있지만, 대신 예측 편향이 생길 수 있다.

## 9.1 (계속) 변수제거의 이론적 결과

### 정리 9.1

행렬
$$\text{Var}(\hat{\beta}_k) - \text{Var}(\tilde{\beta}_k)$$
은 양반정치행렬 (positive semi-definite matrix)이다.

사진 원문에는 정확히 "행렬 $\text{Var}(\tilde{\beta}_k)-\text{Var}(\hat{\beta}_k)$은 양반정치행렬이다"처럼 보일 여지가 있으나, 바로 아래 해설 문맥상 의미는 **완전모형에서 추정된 회귀계수의 분산이 축소모형보다 더 크다**는 것이다. 즉 핵심 해석은 다음과 같다.

* 완전모형에서의 계수 추정은 불편하지만
* 더 많은 변수를 함께 추정해야 하므로 분산이 커질 수 있다.
* 반대로 축소모형에서는 일부 변수를 제거하므로 $\tilde{\beta}_k$의 분산은 더 작아질 수 있다.

**완전모형에서 추정된 회귀계수의 분산은 축소모형에서 추정된 회귀계수의 분산보다 더 커진다.**

### 정리 9.2의 내용

#### 정리 9.2

$$\text{Var}(\hat{y}) \ge \text{Var}(\tilde{y}_k)$$

즉, 완전모형에 의한 $E(y)$의 추정 $\hat{y}$는 축소모형에 의한 추정 $\tilde{y}_k$보다 더 큰 분산을 가진다.

이 정리의 의미는 명확하다.
설명변수를 줄이면 예측식이 단순해지므로 예측량의 변동성이 감소할 수 있다. 즉, **축소모형은 예측값의 분산이 작아지는 장점**이 있다.

하지만 이것만으로 축소모형이 무조건 좋다고 결론낼 수는 없다. 왜냐하면 축소모형은 일반적으로 편향을 가지기 때문이다.

### 정리 9.3의 내용

#### 정리 9.3

만약 행렬
$$\text{Var}(\hat{\beta}_r) - \beta_r\beta_r^T$$
이 양반정치행렬이면,
$$\text{Var}(\hat{y}) - \text{MSE}(\tilde{y}_k)$$
도 양반정치행렬이며, 따라서
$$\text{Var}(\hat{y}) \ge \text{MSE}(\tilde{y}_k)$$
가 성립한다.

이 정리는 변수제거의 이론적 근거를 제시하는 중요한 정리다.

완전모형은 예측에서 불편하지만 분산이 크다.
축소모형은 편향이 있으나 분산이 작다.
그런데 어떤 조건 아래에서는 축소모형의 평균제곱오차가 완전모형의 분산보다 더 작아질 수 있다. 즉, **약간의 편향을 감수하더라도 분산 감소 효과가 더 크다면 축소모형이 오히려 더 좋은 선택**이 된다.

이것이 변수 제거를 정당화하는 이론적 배경이다.

### 평균제곱오차의 해석: 분산과 편향의 제곱의 합

예를 들어 축소모형 예측량 $\tilde{y}_k$의 평균제곱오차는
$$\text{MSE}(\tilde{y}_k)=\text{Var}(\tilde{y}_k)+[\text{Bias}(\tilde{y}_k)]^2$$
로 표현된다고 말한다.

여기서 편향은
$$\text{Bias}(\tilde{y}_k) = E(\tilde{y}_k)-E(y) = E(\tilde{y}_k)-x^T\beta \tag{11.15}$$
이다.

즉, 축소모형이 좋은지 나쁜지는 단순히 편향이 있는지 없는지로 결정되는 것이 아니라,
**편향 증가와 분산 감소 중 어느 효과가 더 큰지**, 곧 평균제곱오차 기준으로 판단해야 한다.

이 대목이 변수선택 (variable selection)의 본질이다.
설명변수를 많이 넣으면 편향은 줄지만 분산이 커지고, 설명변수를 줄이면 분산은 줄지만 편향이 커질 수 있다. 따라서 "좋은 모형"이란 편향과 분산의 균형이 더 나은 모형이다.

### 반대 상황: 참모형이 축소모형일 때

지금까지의 논의는 식 (11.3)의 완전모형이 참모형 (true model)일 때, 즉 제거된 변수들도 사실은 실제 모형에 필요한 변수라고 가정하고 진행한 것이다.

이제 반대로 축소모형
$$y = X_k\beta_k + \epsilon$$
이 참모형이라고 하자. 그러면 완전모형
$$y = X_k\beta_k + X_r\beta_r + \epsilon$$
을 적합한 경우 실제로는
$$\beta_r = 0$$
이다. 따라서
$$E(\hat{\beta}) = \begin{bmatrix} \hat{\beta}_k \\ \hat{\beta}_r \end{bmatrix} = \begin{bmatrix} \beta_k \\ 0_{p-k} \end{bmatrix} \tag{11.16}$$
이므로 $\hat{\beta}_k$는 $\beta_k$의 불편추정량이다. 그리고 축소모형에서의 최소제곱추정량 $\tilde{\beta}_k$ 역시 $\beta_k$의 불편추정량이다.

즉, 이 경우에는 완전모형과 축소모형이 모두 편향이 없다. 하지만 정리 9.1에 의해 축소모형의 분산이 더 작으므로, 평균제곱오차 관점에서도 축소모형이 더 유리하다.

* 참모형에 불필요한 변수를 추가하면 편향은 생기지 않지만 분산이 커진다.
* 따라서 불필요한 변수는 제거한 축소모형이 더 좋은 선택이 될 수 있다.

즉, 변수 선택은 단지 "필요한 변수를 빼면 안 된다"는 문제만이 아니라,
반대로 "불필요한 변수를 넣어도 손해를 볼 수 있다"는 점까지 포함하는 문제이다.


## 9.2 변수선택의 방법 (Methods of Variable Selection)

반응변수 $y$를 설명하기 위해 $p$개의 설명변수 $x_1, x_2, \dots, x_p$가 주어졌다고 하자. 이때 회귀방정식 (regression equation)을 만들기 위해서는 이들 중 일부를 선택하여 모형을 구성해야 한다.

설명변수의 부분집합을 선택하는 경우의 수를 생각하면,

* $p$개 변수 중 1개를 선택하는 방법은 $\binom{p}{1}$개
* 2개를 선택하는 방법은 $\binom{p}{2}$개
* 일반적으로 $k$개를 선택하는 방법은 $\binom{p}{k}$개

이다.

따라서 가능한 모든 부분집합의 총 개수는

$$\sum_{k=0}^{p} \binom{p}{k} = 2^p \tag{11.17}$$

개가 된다.

여기서 $k=0$인 경우는 **모든 설명변수를 제외하고 절편 (intercept)만 사용하는 경우**이다.

* $p$가 작으면 $2^p$개의 모든 회귀모형을 전부 적합해 보는 것이 어렵지 않을 수 있다.
* 그러나 $p$가 커지면 $2^p$개의 모형을 모두 다루는 것은 계산량이 급격히 커져 현실적으로 어려워진다.
* 예를 들어 $p=50$이면 적합할 모형의 수는 대략 $2^{50}\approx 10^{15}$ 수준이 되어, 현대 컴퓨터를 사용해도 전부 탐색하는 것은 사실상 불가능에 가깝다.

따라서 변수선택 (variable selection)은 단순히 "어떤 변수를 넣을 것인가"의 문제가 아니라, **주어진 판정기준 (criterion)** 하에서 **계산 가능한 절차를 통해 최적모형 (best model)** 을 찾는 문제이다.

최적모형을 찾는 절차로 다음 네 가지를 소개한다.

1. 전역탐색법 (Exhaustive search method)
2. 후진제거법 (backward elimination method)
3. 전진선택법 (forward selection method)
4. 단계적 선택법 (stepwise selection method)

이후 절에서 이들을 차례로 설명한다.

### 9.2.1 전역탐색법 (Exhaustive Search Method)

전역탐색법은 **가능한 모든 설명변수 조합에 대해 회귀모형을 적합한 뒤, 그중 가장 좋은 조합을 선택하는 방법**이다. 이를 **최적의 부분집합선택 (best subset selection)** 절차라 한다.

예를 들어 설명변수 수가 $p=10$이라면 가능한 모든 회귀모형의 수는

$$2^{10}=1024$$

이므로, 1024개의 회귀모형을 전부 적합해 보고 비교하는 방식이다.

일반적인 절차는 다음과 같다.

* 우선 설명변수 $p$개 중에서 $k$개를 선택한다고 정한다.
* 그러면 가능한 모형 수는 $\binom{p}{k}$개이다.
* 이들 각각을 모두 적합한다.
* 그중에서 결정계수 $R^2$, 잔차평균제곱 $MSE$, 또는 11.3절에서 소개될 다른 판정기준을 사용하여 최적모형을 고른다.

앞으로 $k$개의 설명변수로 이루어진 모형의 결정계수와 잔차평균제곱을 각각

$$R_k^2,\qquad MSE_k$$

로 표기한다.

또한 $p$개 가운데 $k$개를 선택하는 경우의 수는

$$\binom{p}{k}=\frac{p!}{(p-k)!k!}$$

개라고 다시 적고 있다.

그 다음 중요한 설명이 나온다.
$k$가 **고정**되어 있다면, $R_k^2$를 최대화하는 모형과 $MSE_k$를 최소화하는 모형은 **같은 모형**을 선택한다. 그 이유는 다음 식 때문이다.

$$R_k^2 = 1-\frac{SSE_k}{SST} = 1-\left(\frac{n-k-1}{SST}\right)MSE_k \tag{11.18}$$

여기서

* $SSE_k$: $k$개 설명변수를 사용한 모형의 오차제곱합 (sum of squared errors)
* $SST$: 총제곱합 (total sum of squares)

이다.

이 식에서 $n, k, SST$가 고정되어 있으면 $R_k^2$는 $MSE_k$의 단조함수이다. 따라서

* $MSE_k$가 작을수록 $R_k^2$는 크고,
* $R_k^2$가 클수록 $MSE_k$는 작다.

즉, 같은 $k$ 안에서는 두 기준이 같은 모형을 선택한다.

그러나 $k$를 1에서 $p$까지 바꾸어 가면서 각 $k$에 대해 최적모형을 먼저 고른 뒤, 다시 이들 중 최종모형을 선택할 때는 상황이 달라진다. 그 이유는 다음과 같다.

* $R_k^2$는 변수 수 $k$가 커질수록 대체로 증가한다.
* 따라서 단순 $R^2$만 쓰면 변수 수가 많은 모형을 무조건 더 좋다고 판단하는 경향이 있다.
* 그래서 서로 다른 $k$를 비교할 때는 단순 $R^2$보다 **수정결정계수 (adjusted coefficient of determination)** 를 사용하는 것이 더 적절하다.

수정결정계수를 다음과 같이 준다.

$$R_{ak}^2 = 1-\frac{n-1}{n-k-1}(1-R_k^2)$$

수정결정계수는 $R^2$와 달리 단순히 변수 수 $k$가 증가한다고 해서 자동으로 커지지 않는다. 따라서 **모형 복잡도에 대한 패널티 (penalty)** 를 어느 정도 반영하는 판정기준이라고 볼 수 있다. 바로 이 이유 때문에 수정결정계수가 더 좋은 판정기준이다.

#### 예제 9.2.1

참고문헌의 Held 데이터 (Hald data)를 이용하여 **가능한 모든 회귀를 검토하고 최적회귀모형을 선택하라**

표 11.1의 데이터는 총 13개의 관측값을 가지며, 설명변수는 4개이다.

* $x_1, x_2, x_3, x_4$
* 반응변수 $y$

표의 관측값은 다음과 같다.

| 데이터 번호 | $x_1$ | $x_2$ | $x_3$ | $x_4$ |   $y$ |
| ------ | ----: | ----: | ----: | ----: | ----: |
| 1      |     7 |    26 |     6 |    60 |  78.5 |
| 2      |     1 |    29 |    15 |    52 |  74.3 |
| 3      |    11 |    56 |     8 |    20 | 104.3 |
| 4      |    11 |    31 |     8 |    47 |  87.6 |
| 5      |     7 |    52 |     6 |    33 |  95.9 |
| 6      |    11 |    55 |     9 |    22 | 109.2 |
| 7      |     3 |    71 |    17 |     6 | 102.7 |
| 8      |     1 |    31 |    22 |    44 |  72.5 |
| 9      |     2 |    54 |    18 |    22 |  93.1 |
| 10     |    21 |    47 |     4 |    26 | 115.9 |
| 11     |     1 |    40 |    23 |    34 |  83.8 |
| 12     |    11 |    66 |     9 |    12 | 113.3 |
| 13     |    10 |    68 |     8 |    12 | 109.4 |

설명변수가 4개이므로 가능한 회귀모형의 수는 $2^4=16$개이다.

표 11.2는 16개 모든 모형에 대해 $R_{ak}^2$, $R_k^2$, $MSE_k$를 계산한 결과이다.

정리하면 다음과 같다.

| $k$ | 회귀모형                         | $R_{ak}^2$ | $R_k^2$ | $MSE_k$ |
| --- | ---------------------------- | ---------: | ------: | ------: |
| 0   | $\hat{y}=f(1)$               |          0 |       0 |  226.31 |
| 1   | $\hat{y}=f(x_1)$             |     0.4915 |  0.5339 |  115.06 |
| 1   | $\hat{y}=f(x_2)$             |     0.6360 |  0.6663 |   82.39 |
| 1   | $\hat{y}=f(x_3)$             |     0.2210 |  0.2859 |  176.31 |
| 1   | $\hat{y}=f(x_4)$             |     0.6449 |  0.6745 |   80.35 |
| 2   | $\hat{y}=f(x_1,x_2)$         |     0.9744 |  0.9787 |    5.79 |
| 2   | $\hat{y}=f(x_1,x_3)$         |     0.4578 |  0.5482 |  122.71 |
| 2   | $\hat{y}=f(x_1,x_4)$         |     0.9670 |  0.9725 |    7.48 |
| 2   | $\hat{y}=f(x_2,x_3)$         |     0.8164 |  0.8470 |   41.54 |
| 2   | $\hat{y}=f(x_2,x_4)$         |     0.6161 |  0.6801 |   86.89 |
| 2   | $\hat{y}=f(x_3,x_4)$         |     0.9224 |  0.9353 |   17.57 |
| 3   | $\hat{y}=f(x_1,x_2,x_3)$     |     0.9764 |  0.9823 |    5.35 |
| 3   | $\hat{y}=f(x_1,x_2,x_4)$     |     0.9764 |  0.9823 |    5.33 |
| 3   | $\hat{y}=f(x_1,x_3,x_4)$     |     0.9751 |  0.9813 |    5.65 |
| 3   | $\hat{y}=f(x_2,x_3,x_4)$     |     0.9637 |  0.9728 |    8.20 |
| 4   | $\hat{y}=f(x_1,x_2,x_3,x_4)$ |     0.9736 |  0.9824 |    5.98 |

이 표를 바탕으로 각 $k$별 최적모형을 뽑으면 표 11.3과 같이 된다.

| $k$ | 회귀모형                         | $R_{ak}^2$ | $R_k^2$ | $MSE_k$ |
| --- | ---------------------------- | ---------: | ------: | ------: |
| 0   | $\hat{y}=f(1)$               |          0 |       0 |  226.31 |
| 1   | $\hat{y}=f(x_4)$             |     0.6449 |  0.6745 |   80.35 |
| 2   | $\hat{y}=f(x_1,x_2)$         |     0.9744 |  0.9787 |    5.79 |
| 3   | $\hat{y}=f(x_1,x_2,x_4)$     |     0.9764 |  0.9823 |    5.33 |
| 4   | $\hat{y}=f(x_1,x_2,x_3,x_4)$ |     0.9736 |  0.9824 |    5.98 |

이때,
* $R_k^2$ 값은 $k$가 커질수록 계속 증가한다.
* 따라서 단순 $R^2$ 기준만 쓰면 결국 설명변수가 가장 많은 모형이 유리해진다.
* 그러나 수정결정계수 $R_{ak}^2$ 기준으로 보면, $k=3$에서의 모형 $$\hat{y}=\hat{\beta}_0+\hat{\beta}_1x_1+\hat{\beta}_2x_2+\hat{\beta}_4x_4$$ 이 최적회귀모형으로 선택된다.
* 잔차평균제곱 $MSE_k$ 기준 역시 같은 모형을 선택한다.

즉, 이 예제에서는 **$(x_1, x_2, x_4)$를 포함하고 $x_3$는 제외한 3변수 모형이 최적회귀모형**으로 판단된다.


I can help you convert the inline and display math notation. However, I notice the selection you've provided includes section 9.2.2, which appears to be complete content rather than a placeholder replacement.

If you want me to convert math notation from `[` `]` format to `$` `$` format, here's the corrected section 9.2.2:

### 9.2.2 후진제거법 (Backward Elimination Method)

후진제거법은 전역탐색법과 달리 $2^p$개의 모든 회귀모형을 비교하지 않는다. 따라서 모형비교를 위한 계산량이 크게 줄어들어 $p$가 큰 경우에 매우 유용한 방법이다.

그러나 단점도 있다. 이 방법이 **최적모형을 위한 변수 조합을 찾지 못할 위험**이 있다. 즉, 계산량은 줄지만 전역적으로 최선의 조합을 항상 보장하지는 않는다.

후진제거법의 절차는 다음과 같다:

1. **모든 변수를 포함한 회귀모형**을 적합한다.
2. 현재 모형에 포함된 모든 변수에 대해 **부분 $F$-검정 (partial $F$-test)** 을 실시한다.
3. 가장 작은 부분 $F$-검정 통계량을 $F_L$이라 하고, 유의수준 $\alpha_{drop}$에서의 임계값 $F_C$와 비교한다.
4. 만약 $F_L < F_C$이면, 가장 작은 $F$값을 준 설명변수를 제거하고, 남은 변수들만으로 새 회귀모형을 적합한다. 그 뒤 다시 2단계로 돌아간다.
5. 만약 $F_L \ge F_C$이면, 더 이상 제거할 변수가 없다고 보고 현재 모형에 있는 변수들을 최종 선택한다.

여기서 비교해야 하는 모형 수는:

* 첫 단계에서는 $p$개 변수 각각의 제거 가능성을 비교하므로 $p$개 비교
* 한 변수를 제거한 뒤에는 $p-1$개 비교
* 그다음에는 $p-2$개 비교
* …
* 마지막에는 1개 비교

즉, 비교할 회귀모형의 최대 개수는 $p+(p-1)+\cdots+2+1=\frac{p(p+1)}{2}$ 개이다.

이는 가능한 모든 회귀모형의 수 $2^p$에 비하면 훨씬 적다. 또한 중간에 $F_L\ge F_C$가 성립하면 제거 작업을 중단하므로 실제 비교 횟수는 이보다 더 적어질 수 있다.

$\alpha_{drop}$는 단순히 각 설명변수가 반응변수와 관련이 있는지를 보는 유의수준이 아니다. 현재 다른 변수들의 조합이 주어진 상태에서, 특정 변수가 추가적인 설명력을 가지는지를 검정하는 기준이다. 즉, **설명변수의 조합에 따라 같은 변수의 유의성 평가가 달라질 수 있다**는 뜻이다.

이 때문에 후진제거법에서는 보통 매우 작은 유의수준보다 다소 큰 유의수준을 택하기도 한다. 최적의 변수조합을 찾는 목적이라면 **5%보다 15–20% 수준의 유의수준을 잡는 것이 더 적절하다고 알려져 있다**고 서술한다.

결론적으로 다음처럼 평가한다.

* 후진제거법은 계산량이 적다는 장점이 있다.
* 그러나 전역탐색법에서 사용하는 변수선택 기준을 반드시 최적화한다는 보장은 없다.
* 따라서 "최적모형"보다는 "좋은 모형"을 찾는 절차로 이해하는 것이 적절하다.

또한 주석에서, 부분 $F$-검정 기반 후진제거법은 전역탐색법의 변수선택 기준(예: $R_{ak}^2$ 등)을 직접 최적화하는 이론적 보장이 없다고 덧붙인다. 만약 특정 선택기준을 최적화하려면, 각 단계에서 **부분 $F$-검정치가 최소인 변수**를 제거할 것이 아니라, **해당 기준을 가장 좋게 만드는 변수 제거**를 해야 한다는 점도 지적한다.

#### 부분 $F$-검정과 $t$-검정의 관계

또한 후진제거 절차에서 부분 $F$-검정 대신 $t$-검정을 사용해도 된다고 설명한다.

변수 $x_j$에 대한 $t$-검정 통계량은

$$t=\frac{\hat{\beta}_j}{\sqrt{\widehat{Var}(\hat{\beta}_j)}}$$

이며, 임계값은

$$t_C=t_{\alpha_{drop}/2}(\phi_E)$$

이다. 여기서 $\phi_E$는 잔차항의 자유도이다.

이때

$$F_{\alpha}(1,\phi_E)=\left[t_{\alpha/2}(\phi_E)\right]^2$$

가 성립하므로, 자유도 1인 부분 $F$-검정은 사실상 $t$-검정의 제곱과 같은 정보를 준다. 따라서 변수 하나를 제거할지 판단할 때 두 검정은 본질적으로 동등하다.

#### 부분 $F$-검정 통계량의 계산식

부분 $F$-검정 통계량을 다음과 같이 설명한다.

현재 남아 있는 모든 변수를 포함한 회귀모형을 **완전모형 (full model)** 로 간주하고, 그 회귀제곱합과 잔차제곱합을 각각

$$SSR(F), \qquad SSE(F)$$

라고 하자.

그리고 변수 $x_j$를 제거하여 만든 모형을 **축소모형 (reduced model)** 이라고 하고, 그 회귀제곱합과 잔차제곱합을 각각

$$SSR(R), \qquad SSE(R)$$

라고 하자. 또한 잔차제곱합의 자유도를 각각 $df(F)$, $df(R)$이라고 하자.

그러면 부분 $F$-검정 통계량은

$$F_0 = \frac{(SSR(R)-SSE(F))/(df(F)-df(R))}{SSR(F)/df(F)}$$

로 표기되어 있으나, 문맥상 이는 통상적인 부분 $F$-검정의 형태를 설명하는 것이며, 이어지는 전개에서는 실질적으로 **한 변수 추가의 유의성 검정**, 즉 귀무가설

$$H_0:\beta_j=0$$

을 검정하는 통계량으로 해석한다.

### 9.2.2 후진제거법 (Backward Elimination Method)

후진제거법은 전역탐색법과 달리 $2^p$개의 모든 회귀모형을 비교하지 않는다. 따라서 모형비교를 위한 계산량이 크게 줄어들어 $p$가 큰 경우에 매우 유용한 방법이다.

그러나 단점도 있다. 이 방법이 **최적모형을 위한 변수 조합을 찾지 못할 위험**이 있다고 명시한다. 즉, 계산량은 줄지만 전역적으로 최선의 조합을 항상 보장하지는 않는다.

후진제거법의 절차는 다음과 같다:

1. **모든 변수를 포함한 회귀모형**을 적합한다.
2. 현재 모형에 포함된 모든 변수에 대해 **부분 $F$-검정 (partial $F$-test)** 을 실시한다.
3. 가장 작은 부분 $F$-검정 통계량을 $F_L$이라 하고, 유의수준 $\alpha_{drop}$에서의 임계값 $F_C$와 비교한다.
4. 만약 $F_L < F_C$이면, 가장 작은 $F$값을 준 설명변수를 제거하고, 남은 변수들만으로 새 회귀모형을 적합한다. 그 뒤 다시 2단계로 돌아간다.
5. 만약 $F_L \ge F_C$이면, 더 이상 제거할 변수가 없다고 보고 현재 모형에 있는 변수들을 최종 선택한다.

여기서 비교해야 하는 모형 수가 얼마나 줄어드는지도 설명한다.

* 첫 단계에서는 $p$개 변수 각각의 제거 가능성을 비교하므로 $p$개 비교
* 한 변수를 제거한 뒤에는 $p-1$개 비교
* 그다음에는 $p-2$개 비교
* …
* 마지막에는 1개 비교

즉, 비교할 회귀모형의 최대 개수는

$$p+(p-1)+\cdots+2+1=\frac{p(p+1)}{2}$$

개이다.

이는 가능한 모든 회귀모형의 수 $2^p$에 비하면 훨씬 적다. 또한 중간에 $F_L\ge F_C$가 성립하면 제거 작업을 중단하므로 실제 비교 횟수는 이보다 더 적어질 수 있다.

$\alpha_{drop}$의 의미도 설명한다. 이것은 단순히 각 설명변수가 반응변수와 관련이 있는지를 보는 유의수준이 아니다. 현재 다른 변수들의 조합이 주어진 상태에서, 특정 변수가 추가적인 설명력을 가지는지를 검정하는 기준이다. 즉, **설명변수의 조합에 따라 같은 변수의 유의성 평가가 달라질 수 있다**는 뜻이다.

이 때문에 후진제거법에서는 보통 매우 작은 유의수준보다 다소 큰 유의수준을 택하기도 한다. 최적의 변수조합을 찾는 목적이라면 **5%보다 15–20% 수준의 유의수준을 잡는 것이 더 적절하다고 알려져 있다**고 서술한다.

결론적으로 다음처럼 평가한다.

* 후진제거법은 계산량이 적다는 장점이 있다.
* 그러나 전역탐색법에서 사용하는 변수선택 기준을 반드시 최적화한다는 보장은 없다.
* 따라서 "최적모형"보다는 "좋은 모형"을 찾는 절차로 이해하는 것이 적절하다.

또한 주석에서, 부분 $F$-검정 기반 후진제거법은 전역탐색법의 변수선택 기준(예: $R_{ak}^2$ 등)을 직접 최적화하는 이론적 보장이 없다고 덧붙인다. 만약 특정 선택기준을 최적화하려면, 각 단계에서 **부분 $F$-검정치가 최소인 변수**를 제거할 것이 아니라, **해당 기준을 가장 좋게 만드는 변수 제거**를 해야 한다는 점도 지적한다.

#### 부분 $F$-검정과 $t$-검정의 관계

또한 후진제거 절차에서 부분 $F$-검정 대신 $t$-검정을 사용해도 된다고 설명한다.

변수 $x_j$에 대한 $t$-검정 통계량은

$$t=\frac{\hat{\beta}_j}{\sqrt{\widehat{\text{Var}}(\hat{\beta}_j)}}$$

이며, 임계값은

$$t_C=t_{\alpha_{drop}/2}(\phi_E)$$

이다. 여기서 $\phi_E$는 잔차항의 자유도이다.

이때

$$F_{\alpha}(1,\phi_E)=\left[t_{\alpha/2}(\phi_E)\right]^2$$

가 성립하므로, 자유도 1인 부분 $F$-검정은 사실상 $t$-검정의 제곱과 같은 정보를 준다. 따라서 변수 하나를 제거할지 판단할 때 두 검정은 본질적으로 동등하다.

#### 부분 $F$-검정 통계량의 계산식

부분 $F$-검정 통계량을 다음과 같이 설명한다.

현재 남아 있는 모든 변수를 포함한 회귀모형을 **완전모형 (full model)** 로 간주하고, 그 회귀제곱합과 잔차제곱합을 각각

$$\text{SSR}(F), \qquad \text{SSE}(F)$$

라고 하자.

그리고 변수 $x_j$를 제거하여 만든 모형을 **축소모형 (reduced model)** 이라고 하고, 그 회귀제곱합과 잔차제곱합을 각각

$$\text{SSR}(R), \qquad \text{SSE}(R)$$

라고 하자. 또한 잔차제곱합의 자유도를 각각 $\text{df}(F)$, $\text{df}(R)$이라고 하자.

그러면 부분 $F$-검정 통계량은

$$F_0 = \frac{(\text{SSE}(R)-\text{SSE}(F))/(\text{df}(R)-\text{df}(F))}{\text{SSE}(F)/\text{df}(F)}$$

로 표기되어 있으나, 문맥상 이는 통상적인 부분 $F$-검정의 형태를 설명하는 것이며, 이어지는 전개에서는 실질적으로 **한 변수 추가의 유의성 검정**, 즉 귀무가설

$$H_0:\beta_j=0$$

을 검정하는 통계량으로 해석한다.

#### 예제 9.2.2

앞선 Hald 데이터에 대해 **후진제거법을 사용하여 적절한 회귀방정식을 구하라**

##### 1단계: 모든 변수 포함 모형 적합

먼저 4개의 설명변수 $x_1,x_2,x_3,x_4$를 모두 사용한 회귀방정식은

$$\hat{y}=f(x_1,x_2,x_3,x_4)$$

$$=\hat{\beta}_0+\hat{\beta}_1x_1+\hat{\beta}_2x_2+\hat{\beta}_3x_3+\hat{\beta}_4x_4$$

$$=62.4052+1.5511x_1+0.5102x_2+0.1019x_3-0.1441x_4$$

이다.

이때

$$R^2=0.9824,\qquad \text{MSE}=5.9829$$

이다.

또 $(X^TX)^{-1}$로부터 각 회귀계수의 분산을 구하면

$$\widehat{\text{Var}}(\hat{\beta}_1)=0.5547,\qquad
\widehat{\text{Var}}(\hat{\beta}_2)=0.5239,$$

$$\widehat{\text{Var}}(\hat{\beta}_3)=0.5696,\qquad
\widehat{\text{Var}}(\hat{\beta}_4)=0.5027$$

이다.

따라서 부분 $F$-검정치는 다음과 같다.

$$x_1:\quad F_0=\frac{\hat{\beta}_1^2}{\widehat{\text{Var}}(\hat{\beta}_1)}=4.3376$$

$$x_2:\quad F_0=\frac{\hat{\beta}_2^2}{\widehat{\text{Var}}(\hat{\beta}_2)}=0.4968$$

$$x_3:\quad F_0=\frac{\hat{\beta}_3^2}{\widehat{\text{Var}}(\hat{\beta}_3)}=0.0182$$

$$x_4:\quad F_0=\frac{\hat{\beta}_4^2}{\widehat{\text{Var}}(\hat{\beta}_4)}=0.0413$$

여기서 가장 작은 값은 $x_3$에 대한 부분 $F$-검정치

$$F_L=0.0182$$

이다.

유의수준 $\alpha_{drop}=0.05$에서 기각치는

$$F_C=F_{0.05}(1,8)=5.32$$

인데,

$$F_L < F_C$$

이므로 설명변수 $x_3$를 제거한다.

##### 2단계: $x_3$ 제거 후 모형 적합

이제 남은 설명변수 $x_1,x_2,x_4$에 대해 회귀하면

$$\hat{y}=f(x_1,x_2,x_4)$$

$$=\hat{\beta}_0+\hat{\beta}_1x_1+\hat{\beta}_2x_2+\hat{\beta}_4x_4$$

$$=71.6482+1.4519x_1+0.4161x_2-0.2365x_4$$

이다.

이때

$$\text{MSE}=5.3303$$

이다.

이 모형에서 부분 $F$-검정치를 구하면

$$x_1:\quad F_0=154.0080$$

$$x_2:\quad F_0=5.0259$$

$$x_4:\quad F_0=1.8633$$

이다.

가장 작은 값은 $x_4$에 대한

$$F_L=1.8633$$

이고, 이때 기각치는

$$F_C=F_{0.05}(1,9)=5.12$$

이다. 따라서

$$F_L<F_C$$

이므로 $x_4$도 제거한다.

##### 3단계: $x_1,x_2$만 남긴 모형

이제 남은 두 변수 $x_1,x_2$에 대한 회귀방정식은

$$\hat{y}=f(x_1,x_2)$$

$$=52.5773+1.4683x_1+0.6623x_2 \tag{11.20}$$

이다.

이때

$$R^2=0.9787,\qquad \text{MSE}=5.7904$$

이다.

또 $x_1,x_2$에 대한 부분 $F$-검정치는

$$x_1:\quad F_0=146.5229$$

$$x_2:\quad F_0=208.5823$$

이다.

이 값들이 모두 기각치

$$F_C=F_{0.05}(1,10)=4.96$$

보다 크므로 더는 제거할 변수가 없다고 결론짓는다. 따라서 후진제거법에 의해 **설명변수 $x_1$과 $x_2$가 선택되며**, 최종회귀모형으로는 식 (11.20)이 사용된다.

##### 예제의 해석

이 결과는 전역탐색법의 최적모형과는 다르다.

* 전역탐색법에서는 $(x_1,x_2,x_4)$ 모형이 최적
* 후진제거법에서는 $(x_1,x_2)$ 모형이 선택

즉, 후진제거법은 계산량을 줄이는 대신 **탐색 경로(path dependence)** 에 따라 전역 최적해와 다른 모형을 줄 수 있다. "최적모형을 보장하지 않는다"의 이유가 바로 여기에 있다.


### 9.2.3 전진선택법 (Forward Selection Method)

전진선택법 (forward selection method)은 후진제거법 (backward elimination method)과 정반대 방향의 절차이다.

후진제거법은 **모든 설명변수를 포함한 큰 모형**에서 출발하여 중요하지 않다고 판단되는 설명변수를 하나씩 제거해 나가는 방법이었다. 반면 전진선택법은 **절편 (intercept)만 있는 작은 모형**에서 시작하여, 중요하다고 판단되는 설명변수를 하나씩 추가해 가는 방법이다.

전진선택법의 기본 생각을 다음과 같이 설명한다.

* 처음에는 절편만 있는 모형에서 시작한다.
* 반응변수 $y$와 가장 관련이 큰 설명변수를 하나 선택한다.
* 그 다음에는 이미 선택된 변수와 함께 넣었을 때 설명력을 가장 많이 증가시키는 변수를 하나 더 선택한다.
* 이렇게 한 변수씩 추가하면서, 추가된 변수가 유의한지 검정한다.
* 더 이상 유의하게 추가할 수 있는 변수가 없으면 선택을 중단한다.

즉, 전진선택법은 **모형을 점차 확장하는 방식**의 변수선택 절차이다.

#### 절차 1: 첫 번째 변수 선택

모든 설명변수 중에서 반응변수 $y$와 가장 상관관계가 높은 변수를 선택한다. 이 변수를 $x_k$라고 하자.

이를 위해 각 설명변수 $x_j$, $j=1,2,\dots,p$에 대해 단순회귀모형

$$\hat{y}=f(x_j)$$

를 적합하고, 각 모형의 결정계수 $R^2$를 구한다. 단순회귀에서는 상관계수의 제곱이 결정계수이므로, 결국 $y$와 가장 상관이 큰 설명변수를 찾는 것과 같다.

이들 중 결정계수 $R^2$를 가장 크게 하는 설명변수를 첫 번째 설명변수로 선택한다. 즉, 가장 좋은 단순회귀모형을 주는 변수를 선택한다.

그 다음 이 단순회귀모형

$$\hat{y}=f(x_k)$$

가 유의한지를 유의수준 $\alpha_{add}$에서 $F$-검정한다.

* 만약 유의하지 않으면 더 이상 선택할 변수가 없다고 판단한다.
* 유의하면 다음 단계로 넘어간다.

여기서 $\alpha_{add}$는 **변수를 추가하기 위한 유의수준**이다.

#### 절차 2: 두 번째 변수 선택

이제 아직 선택되지 않은 설명변수 $x_j$, $j\neq k$에 대해 각각 2변수 회귀모형

$$\hat{y}=f(x_k,x_j)$$

를 적합한다.

각 모형의 결정계수 $R^2$를 구하여, 그중 $R^2$ 값을 가장 크게 하는 설명변수를 선택한다. 이 변수를 $x_q$라고 하자.

즉, 이미 선택된 첫 번째 변수 $x_k$에 더하여 넣었을 때 설명력을 가장 많이 증가시키는 설명변수 $x_q$를 찾는 것이다.

그 다음 $x_q$에 대해 유의수준 $\alpha_{add}$에서 **부분 $F$-검정 (partial $F$-test)** 을 실시한다.

* 유의하면 다음 변수 선택을 위해 다음 단계로 넘어간다.
* 유의하지 않으면 $x_q$를 추가하지 않고, 이미 선택된 $x_k$만을 포함한 모형

$$\hat{y}=f(x_k)$$

을 최종모형으로 선택한다.

즉, "설명력 증가가 가장 큰 변수"라고 해도, **기존 변수들이 이미 들어간 상태에서 추가 효과가 유의하지 않으면 채택하지 않는다**는 뜻이다.

#### 절차 3: 세 번째 이후 변수 선택

이제 아직 선택되지 않은 변수들 중에서 하나를 추가하는 문제로 일반화된다.

예를 들어 이미 $x_k$, $x_q$가 선택되어 있을 때, 남아 있는 설명변수 $x_j$, $j\neq k,q$에 대해 각각

$$\hat{y}=f(x_k,x_q,x_j)$$

를 적합한다.

각 모형의 $R^2$를 계산하여 가장 큰 값을 주는 설명변수 $x_j$를 선택하고, 이를 $x_r$라고 하자.

그 다음 $x_r$에 대해 유의수준 $\alpha_{add}$에서 부분 $F$-검정을 실시한다.

* 유의하면 같은 방식으로 또 하나의 변수를 추가한다.
* 유의하지 않으면 선택 절차를 중단하고, 현재까지 선택된 변수들만 포함한 모형을 최종모형으로 선택한다.

즉, 전진선택법은 "설명력 증가가 가장 큰 변수"를 **순차적으로** 넣되, 매 단계마다 **그 증가가 통계적으로 유의한지 확인**하는 절차이다.

#### $\alpha_{add}$의 의미와 계산량

$\alpha_{add}$가 **변수를 추가하기 위한 유의수준**에 해당한다고 설명한다. 이 값 역시 후진제거법의 $\alpha_{drop}$와 마찬가지로 보통 5%보다 훨씬 큰 유의수준을 쓰는 것이 일반적이라고 한다.

또한 전진선택법의 계산량도 후진제거법과 비슷한 수준이라고 설명한다. 최대로 적합해야 할 회귀모형 수는

$$p+(p-1)+\cdots+2+1=\frac{p(p+1)}{2}$$

개 정도이며, 이는 가능한 모든 회귀모형 $2^p$개를 다 적합하는 전역탐색법 (exhaustive search method)에 비해 훨씬 적다. 따라서 $p$가 큰 경우에도 실용적으로 사용할 수 있다.

#### 전진선택법의 한계

후진제거법과 마찬가지로 전진선택법도 **이 방법에 의해 선택된 회귀모형이 '최적'이라는 보장은 없다**고 명시한다.

그 이유를 예를 들어 설명한다. 어떤 회귀모형

$$\hat{y}=f(x_p,x_q,x_r)$$

이 전진선택법에 의해 선택되었다고 하더라도, 3개의 설명변수를 갖는 가능한 모든 모형 $\binom{p}{3}$개를 모두 비교해 보면, 이것보다 $R^2$를 더 크게 하는 다른 조합이 존재할 수 있다.

이런 일이 생기는 이유는 전진선택법이 **처음에 선택된 변수를 이후 단계에서도 계속 고정**시키기 때문이다. 즉, 처음 단계에서 선택된 설명변수 $x_k$는 다음에 어떤 변수가 들어오든 모형에 계속 남아 있게 된다. 따라서 전역적으로는 더 좋은 조합이 있어도, 초기에 선택된 변수가 잘못되면 최적조합을 놓칠 수 있다.

이러한 점을 보완하기 위해 다음 절에서 소개할 방법이 **단계적 전진선택법 (stepwise forward selection method)** 이라고 설명한다.

또한 각주에서는, 전진선택법 역시 후진제거법과 마찬가지로 $R_{ak}^2$ 등 11.3절의 변수선택 기준을 직접 최적화하는 이론적 보장이 없다고 덧붙인다. 만약 특정 선택기준을 직접 최적화하고자 한다면, 각 단계에서 단순히 $R^2$를 가장 크게 하는 변수를 넣는 것이 아니라, 그 판정기준을 가장 좋게 만드는 변수를 선택해야 한다는 뜻이다.

#### 예제 9.2.3

예제 11.3은 Hald 데이터에 대해 전진선택법으로 설명변수를 선택하는 과정이다.

##### 1단계: 첫 번째 변수 선택

표 11.2에서 단일 변수 모형들의 결정계수를 보면

* $\hat{y}=f(x_1)$: $R^2=0.5339$
* $\hat{y}=f(x_2)$: $R^2=0.6663$
* $\hat{y}=f(x_3)$: $R^2=0.2859$
* $\hat{y}=f(x_4)$: $R^2=0.6745$

이므로, 가장 큰 $R^2$를 주는 단순회귀모형은

$$\hat{y}=f(x_4)$$

이다. 따라서 $x_4$가 첫 번째 선택변수이다.

이 단순회귀모형에 대한 분산분석표 (analysis of variance table)는 다음과 같이 주어진다.

* 회귀 제곱합: 1831.8968
* 회귀 자유도: 1
* 회귀 평균제곱: 1831.8698
* 잔차 제곱합: 883.8668
* 잔차 자유도: 11
* 잔차 평균제곱: 80.3515
* $F_0=22.7985$
* 기각치 $F_{0.05}(1,11)=4.84$

따라서 유의수준 $\alpha_{add}=5\%$에서

$$F_0=22.7985 > F_{0.05}(1,11)=4.84$$

이므로 회귀모형 $\hat{y}=f(x_4)$는 유의하며, $x_4$가 첫 번째로 선택된다.

이 단계에서 $R^2$를 일일이 비교하지 않고, 표 11.4의 상관계수 표를 보고 $y$와의 상관이 가장 큰 변수 $x_4$를 택해도 같은 결과를 얻는다고 설명한다.

##### 2단계: 두 번째 변수 선택

이제 $x_4$와 함께 넣을 수 있는 변수는 $x_1, x_2, x_3$이다. 각 2변수 모형의 $R^2$는 다음과 같다.

$$\hat{y}=f(x_1,x_4):\quad R^2=0.9725$$

$$\hat{y}=f(x_2,x_4):\quad R^2=0.6801$$

$$\hat{y}=f(x_3,x_4):\quad R^2=0.9353$$

이 중 가장 큰 $R^2$를 주는 모형은

$$\hat{y}=f(x_1,x_4)$$

이므로, 두 번째 선택변수는 $x_1$이다.

이 회귀모형은

$$\hat{y}=f(x_1,x_4)=103.09738+1.43996x_1-0.61395x_4$$

이다.

이제 변수 $x_1$의 추가가 유의한지 부분 $F$-검정을 실시한다. 다음 계산을 준다.

$$F_0=\frac{\hat{\beta}_1^2}{\widehat{\text{Var}}(\hat{\beta}_1)}=\frac{(1.43996)^2}{0.019159}=108.225$$

기각치는

$$F_{0.05}(1,10)=4.96$$

이므로,

$$108.225 > 4.96$$

이다. 따라서 $x_1$의 추가는 유의하며, $x_1$이 선택된다.

##### 3단계: 세 번째 변수 선택 시도

이제 이미 선택된 변수는 $x_4, x_1$이고, 아직 추가 가능한 변수는 $x_2, x_3$이다.

각 3변수 모형의 결정계수를 보면

$$\hat{y}=f(x_4,x_1,x_2):\quad R^2=0.9823$$

$$\hat{y}=f(x_4,x_1,x_3):\quad R^2=0.9813$$

이므로, $x_2$를 추가하는 것이 더 바람직하다.

따라서 회귀모형은

$$\hat{y}=71.6482+1.4519x_1+0.4161x_2-0.2365x_4$$

이 되고, 이에 대해 변수 $x_2$의 부분 $F$-검정을 하면

$$F_0=\frac{\hat{\beta}_2^2}{\widehat{\text{Var}}(\hat{\beta}_2)}=\frac{(0.4161)^2}{0.03445}=5.0258$$

이다.

기각치는

$$F_{0.05}(1,9)=5.12$$

이므로,

$$5.0258 < 5.12$$

이다. 따라서 유의수준 $\alpha_{add}=0.05$에서는 $x_2$의 추가가 **유의하지 않다**.

따라서 전진선택 절차는 여기서 종료되며, 최종적으로 선택된 변수는 $x_1$과 $x_4$이다. 최종회귀모형은

$$\hat{y}=103.09738+1.43996x_1-0.61395x_4$$

이다.

##### 예제 결과의 해석

이 예제를 통해 후진제거법과 전진선택법이 서로 다른 결과를 줄 수 있음을 보여 준다.

* 후진제거법에서는 $(x_1,x_2)$가 선택되었다.
* 전진선택법에서는 $(x_1,x_4)$가 선택되었다.

즉, 같은 데이터라도 변수선택 절차에 따라 최종모형이 달라질 수 있으므로, 선택 결과를 해석할 때 주의가 필요하다.


### 9.2.4 단계적 전진선택법 (Stepwise Forward Selection Method)

단계적 전진선택법 (stepwise forward selection method)을 **전진선택법을 개선한 방법**으로 소개한다.

전진선택법은 변수를 하나씩 추가해 가지만, 한번 들어간 변수는 이후 단계에서 다시 검토하지 않는다. 그러나 실제로는 새로운 변수가 추가되면, 먼저 들어가 있던 변수의 유의성이 달라질 수 있다. 즉, 처음에는 중요해 보였던 변수가 나중에 다른 변수들이 들어온 후에는 더 이상 유의하지 않을 수 있다.

단계적 전진선택법은 바로 이 점을 보완하기 위한 절차이다.

핵심 아이디어는 다음과 같다.

* 기본적으로는 전진선택법처럼 작은 모형에서 시작하여 변수를 하나씩 추가한다.
* 하지만 새로운 변수가 들어올 때마다, **이미 들어가 있던 변수들이 여전히 유의한지 다시 검토**한다.
* 유의하지 않게 된 변수는 다시 제거한다.

즉, **추가(add)** 와 **제거(drop)** 를 모두 허용하는 방식이다. 그래서 이 방법은 전진선택법과 후진제거법의 절충형으로 볼 수 있다.

#### 절차 1

전진선택법의 순서 (1)과 동일하다.

즉, 모든 설명변수 중에서 반응변수 $y$와 가장 상관이 높거나, 단순회귀의 $R^2$를 가장 크게 하는 변수를 첫 번째로 선택한다. 그 모형이 유의수준 $\alpha_{add}$에서 유의하면 다음 단계로 간다.

#### 절차 2

설명변수 $x_j$, $j\neq k$에 대해 각각

$$\hat{y}=f(x_k,x_j)$$

를 적합하고, $R^2$를 가장 크게 하여 주는 설명변수 $x_j$를 선택한다. 이를 $x_q$라 하자.

그 다음 유의수준 $\alpha_{add}$에서 부분 $F$-검정으로 $x_q$의 추가가 유의한지 검정한다.

* 유의하지 않으면 $\hat{y}=f(x_k)$를 최종모형으로 하여 $x_k$만 선택한다.
* 유의하면 $x_q$를 모형에 추가한다.

그런데 단계적 전진선택법은 여기서 끝나지 않는다. 새로 들어온 변수 $x_q$가 유의하여 채택된 경우, **이미 들어가 있던 변수 $x_k$** 에 대해서도 다시 부분 $F$-검정을 수행한다.

* $x_k$가 여전히 유의하면 남겨 둔다.
* 유의하지 않으면 제거한다.

즉, 두 번째 변수가 추가된 뒤에는 **이전 변수의 생존 여부를 다시 평가**한다.

#### 절차 3

$x_k$와 $x_q$가 모두 유의하여 남아 있는 경우, 다음으로 들어올 설명변수를 선택하기 위해

$$\hat{y}=f(x_k,x_q,x_j), \qquad j\neq k,q$$

를 각각 적합하고, $R^2$를 가장 크게 하는 설명변수 $x_j$를 찾는다. 이를 $x_r$이라 하자.

그 다음 $x_r$에 대해 유의수준 $\alpha_{add}$에서 부분 $F$-검정을 한다.

* 유의하지 않으면 변수선택절차를 중단하고 $x_k,x_q$만 선택한다.
* 유의하면 $x_r$을 모형에 추가한다.

그리고 $x_r$이 추가된 뒤에는, 기존에 들어 있던 $x_k, x_q$에 대해 유의수준 $\alpha_{drop}$에서 부분 $F$-검정을 각각 실시하여, 유의하지 않은 변수가 있으면 제거한다.

또한 앞의 단계 (2)에서 $x_k$가 유의하지 않아 제거되고 $x_q$만 남게 되는 경우도 있다. 그 경우에는 다음에 어떤 변수가 들어갈지를 다시 정하기 위해

$$\hat{y}=f(x_q,x_j),\qquad j\neq q$$

들을 각각 적합하고, $R^2$를 가장 크게 하는 변수를 선택하는 방식으로 계속 진행한다.

즉, 단계적 전진선택법은 경로상에서 모형 구조가 바뀌면 그에 맞추어 다시 다음 후보를 고르는 **동적 절차**이다.

#### $\alpha_{add}$와 $\alpha_{drop}$

단계적 변수선택절차에서

* $\alpha_{add}$
* $\alpha_{drop}$

모두 약 15% 정도의 유의수준을 사용하는 것이 보통이라고 설명한다.

이 절차는 다음이 성립하지 않을 때까지 반복된다.

* 새로 선택되는 변수가 유의하게 추가됨
* 이미 모형에 있는 변수들이 유의하게 남아 있음

즉, 새로 들어올 변수도 충분히 중요해야 하고, 기존 변수들도 새 상황에서 계속 중요해야 한다.

이 방법의 특징을 다음처럼 요약한다.

* 새로 들어온 변수가 유의하면,
* 그 변수 때문에 기존 변수들의 유의성이 변할 수 있으므로,
* 앞서 들어온 변수들이 계속 남아 있을 자격이 있는지 다시 평가한다.

또한 이 절차가 끝난 후 최종모형에 남아 있는 설명변수들에 대해 중회귀모형에서 각 변수의 유의확률은 $\alpha_{drop}$보다 작고, 모형에 포함되지 않은 설명변수들에 대한 유의확률은 $\alpha_{add}$보다 크다고 볼 수 있다고 설명한다.

마지막으로 이 방법이 전진선택법 (forward selection method)을 보완한 방법이며, 적합해야 할 회귀모형의 수 또한 가능한 모든 회귀 (all possible regressions)에 비해 훨씬 적으므로 $k$가 큰 경우에도 활용될 수 있다고 말한다.

#### 예제 9.2.4

예제 11.4는 Hald 데이터에 대해 단계적 선택법을 적용하는 과정이다.

##### $\alpha_{add}=0.05$인 경우

앞의 예제 11.3에서 이미 보았듯이, 처음에는 $x_4$가 선택되고, 다음으로 $x_1$의 선택이 유의하다. 따라서 현재 모형은

$$\hat{y}=f(x_1,x_4)=103.09738+1.43996x_1-0.61395x_4$$

이다.

이때 잔차평균제곱은

$$\text{MSE}=7.47621$$

이다.

이제 새로 들어온 $x_1$ 때문에 기존 변수 $x_4$가 여전히 유의한지를 부분 $F$-검정한다. 계산은

$$F_0=\frac{\hat{\beta}_4^2}{\widehat{\text{Var}}(\hat{\beta}_4)}=\frac{(-0.61395)^2}{0.002366}=159.31$$

이다.

기각치는

$$F_{0.05}(1,10)=4.96$$

이므로,

$$159.31 > 4.96$$

이다. 따라서 $x_4$도 유의하게 남아 있다.

다음으로 $(x_4,x_1)$이 들어간 상태에서 추가할 수 있는 변수 $x_j$, $j\neq 1,4$ 중 $R^2$를 더 크게 하는 변수를 고르면 표 11.2로부터 $x_2$가 선택된다. 회귀모형은

$$\hat{y}=f(x_4,x_1,x_2)=71.6482+1.4519x_1+0.4161x_2-0.2365x_4$$

이고,

$$\text{MSE}=5.330297$$

이다.

이제 변수 $x_2$에 대한 부분 $F$-검정을 해 보면, 앞의 예제와 같이 $\alpha_{add}=0.05$에서 유의하지 않다. 따라서 $\alpha_{add}=0.05$에서는 $x_2$를 추가하지 않게 된다.

그 결과 단계적 선택법에서도 최종적으로 선택된 변수는 $x_1$과 $x_4$가 되며, 회귀모형은

$$\hat{y}=103.09738+1.43996x_1-0.61395x_4$$

이다.

##### $\alpha_{add}=0.10$인 경우

유의수준을 $\alpha_{add}=0.10$으로 바꾸면 결과가 달라진다고 설명한다.

앞서 계산한 $x_2$의 부분 $F$-검정치는

$$F_0=\frac{\hat{\beta}_2^2}{\widehat{\text{Var}}(\hat{\beta}_2)}=5.0258$$

이다.

이때 기각치는

$$F_{0.10}(1,9)=3.36$$

이므로,

$$5.0258 > 3.36$$

이다. 따라서 이번에는 $x_2$의 추가선택이 유의하다.

이제 이미 들어가 있는 $x_4$와 $x_1$에 대해 부분 $F$-검정을 다시 한다.

먼저 $x_4$에 대한 부분 $F$-검정결과는

$$F_0=\frac{\hat{\beta}_4^2}{\widehat{\text{Var}}(\hat{\beta}_4)}=\frac{(-0.2365)^2}{0.03003}=1.86$$

으로 유의하지 않다. 따라서 $x_4$는 제거된다.

반면 $x_1$에 대한 부분 $F$-검정은

$$F_0=\frac{\hat{\beta}_1^2}{\widehat{\text{Var}}(\hat{\beta}_1)}=\frac{(1.4519)^2}{0.01369}=153.98$$

으로 매우 큰 값이므로 유의하다. 따라서 현재 남아 있는 변수는 $x_1$과 $x_2$가 된다.

이제 다음으로 추가될 변수를 다시 찾아보면 $x_4$가 후보가 되지만, 이 변수의 추가는 유의하지 않으므로 선택절차는 여기서 중단된다.

결국 최종적으로 선택된 변수는 $x_1$과 $x_2$이며, 최종회귀모형은

$$\hat{y}=f(x_1,x_2)=52.5773+1.4683x_1+0.6623x_2$$

#### 단계적 선택법 예제의 의미

이 예제는 단계적 선택법의 중요한 특징을 매우 잘 보여 준다.

* 전진선택법에서는 한번 들어온 변수는 이후 제거되지 않는다.
* 단계적 전진선택법에서는 새 변수가 들어온 뒤 기존 변수의 유의성을 다시 검사한다.
* 따라서 이미 선택된 변수라도 나중에 제거될 수 있다.

실제로 이 예제에서는 처음에는 $x_4$가 선택되었지만, $x_1$, $x_2$가 차례로 들어오고 난 뒤에는 $x_4$가 더 이상 유의하지 않아 제거된다. 이 점이 전진선택법과 단계적 전진선택법의 핵심 차이이다.

각주: **단계적 후진제거법 (stepwise backward elimination method)** 이는 단계적 전진선택법과 비슷한 절차로, 후진제거법의 순서 (1)에서 출발하여 제거절차를 거친 뒤 다시 남은 변수 중 선택할 변수가 있는지 검정함으로써, 후진제거법과 전진선택법을 번갈아 사용하는 방식이다.



## 9.3 변수선택의 판단기준 (Criteria for Variable Selection)

앞 절에서는 변수선택법 (variable selection methods)으로 네 가지 절차를 검토하였다. 그때는 주로 계산절차를 중심으로 설명하였고, 결정계수 $(R^2)$를 판단기준 (criterion)으로 사용하여 변수를 선택하였다. 그러나 실제 변수선택에서는 $(R^2)$ 이외에도 여러 판단기준이 사용될 수 있다.

다음 네 가지 판단기준을 제시한다.

1. 잔차평균제곱 (residual mean square)
2. 결정계수 (coefficient of determination)
3. 수정결정계수 (adjusted coefficient of determination)
4. 말로우 (Mallows)의 $C_k$ 통계량

각각의 정의는 다음과 같다.

### 기본 정의

설명변수 $p$개 중 $k(\le p)$개를 선택하여 회귀모형을 적합한 경우를 생각하자. 이때 다음 기호를 사용한다.

* $\text{SSE}_k$: $k$개의 설명변수를 사용한 모형의 오차제곱합 (sum of squared errors)
* $\text{MSE}_k$: 그 모형의 잔차평균제곱 (residual mean square)
* $R_k^2$: 그 모형의 결정계수
* $R_{ak}^2$: 그 모형의 수정결정계수
* $\hat{\sigma}^2$: $p$개의 설명변수를 모두 사용하여 적합한 완전모형 (full model)에서 구한 $\text{MSE}$

그러면 네 판단기준은 다음과 같이 정의된다.

#### (1) 잔차평균제곱 (Residual Mean Square)

$$\text{MSE}_k=\frac{\text{SSE}_k}{n-k-1} \tag{11.21}$$

#### (2) 결정계수 (Coefficient of Determination)

$$R_k^2 = 1-\frac{\text{SSE}_k}{\text{SST}} \tag{11.22}$$

여기서 $\text{SST}$는 총제곱합 (total sum of squares)이다.

#### (3) 수정결정계수 (Adjusted Coefficient of Determination)

$$R_{ak}^2 = 1-\left(\frac{n-1}{n-k-1}\right)(1-R_k^2) \tag{11.23}$$

#### (4) 말로우 (Mallows)의 $C_k$ 통계량

$$C_k=\frac{\text{SSE}_k}{\hat{\sigma}^2}+2(k+1)-n \tag{11.24}$$

이제 이들 판단기준의 의미를 차례로 설명한다.

### 9.3.1 $\text{MSE}_k$, $R_k^2$, $R_{ak}^2$

반응변수 $y$의 변화를 설명하기 위하여 $p$개의 설명변수 중 $k$개를 선택한다고 하자. 이때 결정계수 $R_k^2$를 크게 하여 주는 $k$개의 설명변수를 선택하는 것은 가장 먼저 떠올릴 수 있는 자연스러운 방법이다. 왜냐하면 결정계수는 총변동 ($\text{SST}$) 중에서 회귀식에 의해 설명되는 변동의 비율을 의미하기 때문이다. 따라서 $R_k^2$가 크다는 것은 회귀모형에 의해 설명되지 않는 변동, 즉 $\text{SSE}_k$가 작다는 뜻이다.

이 관계를 바탕으로 $R_k^2$와 $\text{MSE}_k$를 비교한다.

잔차평균제곱은

$$\text{MSE}_k=\frac{\text{SSE}_k}{n-k-1}$$

이므로, $\text{SSE}_k$를 작게 하면 일반적으로 $\text{MSE}_k$도 작아진다. 그러나 여기서 중요한 차이점이 있다.

* $R_k^2$는 설명변수 수 $k$가 증가할수록 감소하지 않는 함수이다.
* 반면 $\text{MSE}_k$는 $k$에 대한 단순 증가함수도 아니고 단순 감소함수도 아니다.

그 이유는 다음과 같다.

* $k$가 증가하면 모형에 더 많은 변수를 넣으므로 $\text{SSE}_k$는 감소한다.
* 그러나 동시에 분모 $n-k-1$도 감소한다.

따라서 $\text{MSE}_k$는 무조건 줄어들지도 않고 무조건 늘어나지도 않는다. 즉, $\text{MSE}_k$를 최소로 하는 어떤 $k$의 값 ($1\le k\le p$)이 존재할 수 있다. 이 점에서 $\text{MSE}_k$는 서로 다른 변수 수를 갖는 모형들을 비교하는 판단기준으로 사용할 수 있다.

반면 $R_k^2$는 변수 수가 증가하면 대체로 증가하므로, 모형 간 비교의 판단기준으로는 적절하지 않다. 설명변수를 더 많이 넣을수록 늘 유리하게 보이기 때문이다.

이 문제를 보완하기 위해 수정결정계수 $(R_{ak}^2)$가 사용된다. 식 (11.23)을 다음과 같이 다시 정리한다.

$$R_{ak}^2 = 1-\left(\frac{n-1}{n-k-1}\right)(1-R_k^2)$$

또한 $1-R_k^2=\dfrac{\text{SSE}_k}{\text{SST}}$ 이므로

$$R_{ak}^2 = 1-\left(\frac{n-1}{n-k-1}\right)\frac{\text{SSE}_k}{\text{SST}}$$

그리고 $\text{MSE}_k=\dfrac{\text{SSE}_k}{n-k-1}$ 이므로

$$R_{ak}^2 = 1-\frac{\text{MSE}_k}{\text{SST}/(n-1)} \tag{11.25}$$

가 된다.

이 식의 의미는 매우 중요하다. $\text{SST}/(n-1)$은 주어진 데이터에서 상수이므로, $R_{ak}^2$를 최대화하는 것은 곧 $\text{MSE}_k$를 최소화하는 것과 같다. 따라서 다음과 같은 결론을 준다.

* $\text{MSE}_k$를 최소로 하는 $k$의 값은 $R_{ak}^2$도 최대화한다.
* 따라서 수정결정계수 $R_{ak}^2$는 $R_k^2$를 대신하여 모형 비교의 판단기준으로 사용될 수 있다.

즉, 서로 다른 변수 수를 가진 모형을 비교할 때는

* 단순 $R^2$보다는
* $\text{MSE}_k$ 또는 $R_{ak}^2$

를 사용하는 것이 더 적절하다.



### 9.3.2 말로우 (Mallows)의 $C_k$

식 (11.24)의 $C_k$ 판단기준도 $\text{SSE}_k$의 함수로 표현되지만, 앞서 논의한 세 판단기준 ($\text{MSE}_k$, $R_k^2$, $R_{ak}^2$)와는 다른 의미를 가진다고 설명한다.

이 기준의 이론적 배경은 **예측 오차 (prediction error)** 또는 보다 정확히는 $E(y)$를 얼마나 잘 추정하는가의 관점에서 나온다.

앞 절의 식 (11.2)에서 $E(y)=x^T\beta$의 추정을 $k$개의 설명변수로 할 때의 추정량을

$$\tilde{y}_{ik}=x_{ik}^T \tilde{\beta}_k$$

로 표현하였다. 여기서 $\tilde{y}_{ik}$는 축소모형 (reduced model)에서 얻은 $i$번째 점의 예측값이다.

$\tilde{y}_{ik}$가 편의추정량, 즉 편향 (bias)을 가질 수 있으므로 $E(y)$를 얼마나 잘 추정하는지를 보려면 $\tilde{y}_{ik}$의 평균제곱오차 (mean square error)를 보는 것이 좋다고 설명한다. 앞 절 식 (11.14)로부터

$$\text{MSE}(\tilde{y}_{ik})=x_{ik}^T(X_k^TX_k)^{-1}x_{ik}\sigma^2+(x_{ik}^T A\beta_r - x_{ir}^T\beta_r)^2 \tag{11.26}$$

이다.

여기서

* 첫 번째 항은 분산 (variance) 부분
* 두 번째 항은 편향 제곱 (squared bias) 부분

이다.

$n$개의 자료점 (data points) 전체에 대해 이 평균제곱오차를 더한 뒤 $\sigma^2$로 나눈 양을 $\Gamma_k$라고 정의한다.

$$\Gamma_k=\frac{1}{\sigma^2}\sum_{i=1}^n \text{MSE}(\tilde{y}_{ik})$$

식 (11.26)을 대입하면

$$\Gamma_k=\frac{1}{\sigma^2}\sum_{i=1}^n\left[x_{ik}^T(X_k^TX_k)^{-1}x_{ik}\sigma^2+(x_{ik}^T A\beta_r - x_{ir}^T\beta_r)^2\right]$$

이 된다.

이제 이 식의 각 항을 정리한다.

먼저 첫 번째 항에 대하여

$$\sum_{i=1}^n x_{ik}^T(X_k^TX_k)^{-1}x_{ik}$$

를 trace를 이용해 전개하면

$$\sum_{i=1}^n x_{ik}^T(X_k^TX_k)^{-1}x_{ik}=\sum_{i=1}^n \operatorname{tr}\left[x_{ik}^T(X_k^TX_k)^{-1}x_{ik}\right]$$

$$=\sum_{i=1}^n \operatorname{tr}\left[(X_k^TX_k)^{-1}x_{ik}x_{ik}^T\right]$$

$$=\operatorname{tr}\left[(X_k^TX_k)^{-1}\sum_{i=1}^n x_{ik}x_{ik}^T\right]$$

그런데

$$\sum_{i=1}^n x_{ik}x_{ik}^T = X_k^TX_k$$

이므로

$$\operatorname{tr}\left[(X_k^TX_k)^{-1}(X_k^TX_k)\right]=\operatorname{tr}(I)=k+1 \tag{11.27}$$

이 된다. 여기서 $k+1$이 되는 이유는 절편까지 포함한 모수의 수가 $k+1$이기 때문이다.

다음으로 두 번째 항은

$$\sum_{i=1}^n (x_{ik}^T A\beta_r - x_{ir}^T\beta_r)^2$$

이며, 이를 전개하면

$$\sum_{i=1}^n\left(\beta_r^T A x_{ik}x_{ik}^T A\beta_r-2\beta_r^T x_{ir}x_{ik}^T A\beta_r+\beta_r^T x_{ir}x_{ir}^T \beta_r\right) \tag{11.28}$$

이 된다.

여기서

$$A=(X_k^TX_k)^{-1}X_k^TX_r,\qquad\sum_{i=1}^n x_{ir}x_{ik}^T = X_r^TX_k,\qquad\sum_{i=1}^n x_{ir}x_{ir}^T = X_r^TX_r$$

이므로 식 (11.28)은 정리되어

$$\beta_r^T X_r^T X_r\beta_r-\beta_r^T X_r^T X_k (X_k^TX_k)^{-1} X_k^T X_r\beta_r$$

가 된다.

따라서 $\Gamma_k$는

$$\Gamma_k=(k+1)+\frac{1}{\sigma^2}\beta_r^T X_r^T\left[I - X_k(X_k^TX_k)^{-1}X_k^T\right]X_r\beta_r \tag{11.29}$$

이 된다.

한편 앞 절 식 (11.11)에 의해

$$E[(n-k-1)\tilde{\sigma}^2]=E(\text{SSE}_k)=(n-k-1)\sigma^2+\beta_r^T X_r^T\left[I - X_k(X_k^TX_k)^{-1}X_k^T\right]X_r\beta_r$$

이다.

따라서 위 식과 비교하면

$$\Gamma_k=\frac{E(\text{SSE}_k)}{\sigma^2}+2(k+1)-n \tag{11.30}$$

을 얻는다.

이제 $\sigma^2$ 대신 그 불편추정값 $\hat{\sigma}^2$를 대입하고, $E(\text{SSE}_k)$ 대신 관측값 $\text{SSE}_k$를 사용한 $\Gamma_k$의 추정값을 $C_k$라고 하면

$$C_k=\frac{\text{SSE}_k}{\hat{\sigma}^2}+2(k+1)-n \tag{11.31}$$

이 된다. 이것이 바로 Mallows의 $C_k$ 통계량이다.

#### $C_k$의 해석

$C_k$의 기대값을 이용하여 이 판단기준을 해석한다.

만약

* $X_k^T X_r = O$, 즉 선택된 변수와 버려진 변수들이 직교 (orthogonal)하거나
* $\beta_r=0$, 즉 버려진 변수들의 실제 효과가 0

이라는 가정이 성립하면

$$E(\text{SSE}_k)=(n-k-1)\sigma^2$$

이 된다. 이 경우 $C_k$의 기대값은 근사적으로

$$E(C_k\mid X_k^TX_r=O \text{ 또는 } \beta_r=0)\approx\frac{(n-k-1)\sigma^2}{\sigma^2}+2(k+1)-n=k+1$$

이 된다.

즉, $p$개의 변수 중 $k$개를 선택하고 $r$개를 버릴 때, 버린 변수들을 적절히 잘 선택하여 $\beta_r=0$에 가까운 상황이면 $C_k$의 값은 $k+1$에 가까워진다.

그래서 일반적으로

$$C_k \le k+1$$

이면 좋은 모형으로 판단한다고 설명한다. 그리고 이 조건을 만족하는 여러 모형이 있다면, **변수의 수가 적은 모형**을 선택하는 것이 바람직하다고 한다.

또한 이 모형선택 기준은 앞 절에서 다룬 네 가지 변수선택 절차

* 모든 가능한 회귀 (all possible regressions)
* 후진제거법 (backward elimination)
* 전진선택법 (forward selection)
* 단계적 선택법 (stepwise selection)

모두와 함께 사용할 수 있다고 설명한다.


## 9.4 혼합모형에서의 변수 선택 (Variable Selection in Mixed Models)

여기서 "혼합모형"은 일반적인 현대 통계에서 말하는 **랜덤효과 혼합모형 (mixed effects model)** 이 아니라, **혼합비율이 1이 되도록 제약된 조성자료의 선형혼합모형 (mixture model)** 을 뜻한다.
chatgpt의 의견임: 현재 통계학 문헌에서 "mixed model"은 보통 랜덤효과 모형을 의미하므로, 이 절의 혼합모형은 문맥상 "mixture model"로 이해해야 한다.

혼합모형 (mixture model)을 다음 제약조건 (constraints)을 가진 회귀모형으로 정의한다.

$$x_1+x_2+\cdots+x_p=1,\qquad x_j\ge 0 \quad (j=1,2,\dots,p) \tag{11.32}$$

즉, $p$개의 설명변수들이 비율 또는 조성 (composition)을 나타내며, 합이 1이 되는 상황이다.

이때 선형혼합모형 (linear mixture model)은

$$E(y)=\beta_1x_1+\beta_2x_2+\cdots+\beta_p x_p$$

의 형태를 가진다.

여기서 관심은 $p$개의 성분 (components) 중 어떤 성분들이 중요한 성분 (essential components)인가를 선택하는 것이다.

다음과 같이 설명한다.

* 일반적인 중회귀모형의 변수선택방법도 어느 정도 적용할 수 있다.
* 그러나 혼합모형은 식 (11.32)의 제약조건을 가지므로, 혼합모형에서만 사용 가능한 특수한 변수선택방법도 존재한다.
* 여기서는 Snee와 Marquardt가 제안한 방법을 간략히 소개한다.

그들은 선형혼합모형에서 변수의 수를 줄이는 기준으로 다음 두 가지를 제시하였다.

### 기준 1: 선형효과 (Linear Effect)가 0인 경우

$j$번째 회귀계수의 추정값 $\hat{\beta}_j$가 나머지 회귀계수 추정값들의 평균과 같다고 하자. 즉,

$$E_j = \hat{\beta}_j - \frac{\sum_{k\ne j} \hat{\beta}_k}{p-1}$$

가 0일 때, 변수 $x_j$는 모형으로부터 제거시킨다.

이 $E_j$를 변수 $x_j$의 선형효과 (linear effect)라고 부르기도 한다. 만약 $E_j=0$이면, 심플렉스 (simplex)에서 $x_j=0$이 되는 선에 수직을 이루는 직선상의 모든 점에서 $\hat{y}$의 값은 변함이 없다. 즉, 그 변수 $x_j$는 반응값에 실질적인 변화를 주지 못하므로 제거할 수 있다는 뜻이다.

#### 기하학적 설명 예시

그림 11.1과 그림 11.2를 통해 이를 설명한다.

예를 들어 추정된 회귀모형이

$$\hat{y}=80x_1+90x_2+100x_3$$

이라고 하자. 그러면 $x_2$의 선형효과는

$$E_2 = \hat{\beta}_2-\frac{\hat{\beta}_1+\hat{\beta}_3}{2} = 90-\frac{80+100}{2} = 0$$

이므로 변수 $x_2$는 제거할 수 있다.

왜냐하면 이 모형에서 $\hat{y}=90$을 주는 등고선표를 보면, $x_2=0$을 주는 직선 ($x_1$의 꼭짓점과 $x_3$의 꼭짓점을 연결한 선)에 수직인 선상에서는 $\hat{y}$가 일정한 값을 갖기 때문이다. 예를 들어 $\hat{y}=90$을 주는 직선상에서는 $x_2$가 0에서 1까지 변할 때 $x_1$과 $x_3$는 동일하게

$$x_1=x_3=\frac{1-x_2}{2}$$

의 좌표를 가진다. 이 직선 위에서는 $x_2$의 값이 달라져도 $\hat{y}$가 계속 90으로 유지된다. 따라서 $x_2$는 효과가 없다고 판단할 수 있다.

즉, $x_2$의 변화가 반응변수 예측값을 변화시키지 않으면 $x_2$는 제거 가능한 성분이다.

### 기준 2: 동일한 효과 (Equal Effects)를 가지는 경우

만약 두 개 이상의 회귀계수 추정값 $\hat{\beta}_j$들이 서로 같다면, 그에 해당하는 변수들은 동등한 효과 (equal effects)를 가진다고 말한다. 이 경우 그 변수들을 한데 묶어 하나의 새로운 변수로 만들어 변수의 수를 줄인다.

#### 기하학적 설명 예시

예를 들어 추정된 회귀모형이

$$\hat{y}=90x_1+80x_2+80x_3$$

이라고 하자. 그러면

$$\hat{\beta}_2=\hat{\beta}_3=80$$

이므로 $x_2$와 $x_3$는 동등한 효과를 가진다.

이때

$$x_2^* = x_2+x_3$$

로 두면, 추정식은

$$\hat{y}=90x_1+80x_2^*$$

로 쓸 수 있다. 즉, $x_2$와 $x_3$는 어떤 비율로 섞이든 무방하며, 그 합 $x_2^*$가 하나의 새로운 변수처럼 행동한다. 물론 이때도 제약조건은

$$x_1+x_2^*=1$$

이다.

그림 11.2를 통해 이 사실을 설명한다. 예를 들어 $\hat{y}=85$를 주는 선상에서는 $x_1=0.5$, $x_2+x_3=0.5$가 된다. 이때 $x_2$와 $x_3$의 개별값은 달라져도 합 $x_2+x_3$만 유지되면 $\hat{y}$는 변하지 않는다. 따라서 $x_2$와 $x_3$는 하나의 변수로 묶어도 된다고 해석할 수 있다.

즉, 혼합모형에서는 어떤 성분들이 서로 구분되지 않는 동일 효과를 보이면 그들을 통합하여 차원을 줄일 수 있다.

### 실제 분석에서는 가설검정을 사용

실제 문제에서 $\hat{\beta}_j$들이 위 두 기준을 **정확하게** 만족하는 경우를 찾기는 어렵다고 설명한다. 따라서 실제로는 다음과 같은 가설검정 (hypothesis testing)을 통해 변수제거 또는 변수통합을 수행할 수 있다.

#### (1) 선형효과가 0인지에 대한 검정

먼저

$$H_0: \beta_j - \frac{\sum_{k\ne j}^{p}\beta_k}{p-1} = 0$$

을 검정한다.

이것이 7장에서 다룬 다른 가설검정의 $C\beta=0$ 형태이므로 어렵지 않게 검정을 실시할 수 있다고 설명한다. 만약 이 귀무가설이 채택되면 $x_j$를 제거할 수 있다.

#### (2) 두 성분의 효과가 같은지 검정

다음으로

$$H_0:\beta_j-\beta_k=0,\qquad j\ne k$$

에 대한 검정을 할 수 있다.

이 귀무가설이 채택되면 $x_j$와 $x_k$를 묶어 하나의 변수로 취급할 수 있다.

#### (3) 세 성분 이상의 효과가 같은지 검정

또는

$$H_0:\beta_j=\beta_k=\beta_l,\qquad j\ne k\ne l$$

에 대한 검정도 같은 방법으로 실시할 수 있다.

이 귀무가설이 채택되면 $x_j, x_k, x_l$를 합하여 하나의 변수로 취급할 수 있다.

마지막으로 혼합모형에 대해서는 Cornell이 상세한 문헌조사를 하였다고 언급하며, 혼합모형을 더 공부하려는 독자에게 도움이 될 것이라고 덧붙인다.
