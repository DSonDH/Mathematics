...
라그랑주 승수법은 **제약조건이 있는 최적화 문제를 제약이 없는 형태로 바꾸어 푸는 방법**으로, 최적점이 만족해야 하는 방정식을 세워 후보점을 찾는 방법이다.
...
제약식 $g(x) = 0$은 곡선이나 곡면을 정의한다. 최적점에서는 제약을 지키면서 조금 움직일 때 목적함수 $f$가 더 이상 커지지 않아야 한다. 다시 말해, $f$를 늘리려는 방향($\nabla f$)이 제약면과 수직으로 만나야 한다는 뜻이다. 수직으로 만나지 않는다면 $f$값이 더 크거나 작은 점이 근처에 존재하는 것이므로 국소최댓값이나 국소최솟값이 될 수 없다.
...
위 조건을 체계적으로 계산하기 위해 라그랑지안을 정의한다: 최적점이 만족해야 하는 방정식을 세우는 것이다.
