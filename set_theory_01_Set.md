...
# 집합 (Set)
Set: 원소(element)라 불리는 대상의 모임.

주어진 집합 $A$에 대해 $x$가 $A$의 원소를 기호로 표현하면: $x \in A$  
원소가 아닌 경우를 기호로 표기하면 $x \notin A$

- 합집합: $A\cup B=\{x\mid x\in A\lor x\in B\}$.  
  - $A$ 또는 $B$ 중 적어도 하나에 속하는 원소들의 집합.  
  - 예: $A=\{1,2,3\},\;B=\{3,4,5\}$이면 $A\cup B=\{1,2,3,4,5\}$.

- 교집합: $A\cap B=\{x\mid x\in A\land x\in B\}$.  
  - $A$와 $B$ 양쪽 모두에 속하는 원소들의 집합.  
  - 예: $A=\{1,2,3\},\;B=\{3,4,5\}$이면 $A\cap B=\{3\}$.

- 공집합(empty set): 원소가 하나도 없는 집합. 
  - $\varnothing$ 또는 $\{\}$로 표기. 모든 집합의 부분집합이다.

- 서로소(disjoint): $A\cap B=\varnothing$인 두 집합 $A,B$. 
  - 공통 원소가 없음.

- 포함(containment): $A\subseteq B \iff \forall x,\;x\in A\Rightarrow x\in B$. 
  - "$A$는 $B$에 포함된다."

- 부분집합(subset): $A\subseteq B$이면 $A$는 $B$의 부분집합.  
- 진부분집합(proper subset): $A\subsetneq B \iff A\subseteq B$이고 $A\neq B$.

TODO: 
여집합  
드모르간 법칙  
전칭기호(for all, ...), 존재기호를 잘 다룰 수 있어야 함.

## 집합족
- 집합족(family of sets): 집합을 원소로 갖는 집합(예: 멱집합).  
- 첨수족(indexed family): 각 원소에 첨자(인덱스)가 부여된 집합들의 모임. 
  - 예를 들어 $A$의 멱집합이 $\mathcal P(A)=\{\varnothing,\{1\},\{2\},\{1,2\}\}$일 때, 원소들에 인덱스를 매겨 $\{B_i\mid i\in I\}$처럼 표현할 수 있다.

### 집합족의 연산
- 합집합: $\displaystyle\bigcup\mathcal F=\{x\mid\exists A\in\mathcal F,\;x\in A\}$.  
  - 즉, $\mathcal F$에 속한 적어도 하나의 집합이 $x$를 포함하면 $x$는 합집합에 속한다.  
  - 예: $\mathcal F=\{\{1,2\},\{2,3,4\},\{5\}\}\Rightarrow \bigcup\mathcal F=\{1,2,3,4,5\}$.

- 교집합: $\displaystyle\bigcap\mathcal F=\{x\mid\forall A\in\mathcal F,\;x\in A\}$.  
  - 즉, $\mathcal F$에 속한 모든 집합에 공통으로 들어있는 원소들의 집합이다.  
  - 예: $\mathcal F=\{\{1,2,3\},\{2,3,4\},\{2,3,5\}\}\Rightarrow \bigcap\mathcal F=\{2,3\}$.

## 곱집합 (카르테시안 곱, Cartesian product)
두 집합의 순서쌍들로 이루어진 집합.  
두 집합 $A,B$에 대해 $A\times B=\{(a,b)\mid a\in A,\;b\in B\}$.
...
- 교환성(정확히는 자연적인 전단사): $A\times B\cong B\times A$  
(순서쌍의 성분을 바꾸는 전단사를 통해), 하지만 집합으로서 동치(순서가 다름)는 아니다.
- 결합성(자연동형): $(A\times B)\times C\cong A\times(B\times C)$  
(표기상의 차이를 제외하면 동일하게 다룰 수 있다).
...
- 부분집합족과의 관계
  - 각 성분에 대해 부분집합 관계가 주어지면(즉 $ \forall i, B_i\subseteq A_i$): $\prod_i B_i\subseteq\prod_i A_i$가 성립한다.
