...
$$ mgf_U(t) = mgf_V(f)mgf_W(t)$$
한편, $(X_i-\mu)/\sigma$는 서로 독립이고 표준정규분포를 따르므로
$$
U = \sum_{i=1}^n\left(\frac{X_i-\mu}{\sigma}\right)^2 \sim \chi^2(n) \quad \text{즉 } mgf_U(t) = (1-2t)^{-n/2}I_{(t<1/2)}
$$
또한 (a)로부터 $(\bar X-\mu)/(\sigma/\sqrt n)$도 표준정규분포를 따르므로
$$
W= \left(\frac{\bar X-\mu}{\sigma/\sqrt n}\right)^2 \sim \chi^2(1) \text{즉 } mgf_W(t) = (1-2t)^{-1/2}I_{(t<1/2)} \\
\therefore mgf_V(t) = \frac{mgf_U(t)}{mgf_W(t)} = (1-2t)^{-(n-1)/2}I_{(t<1/2)} \\
\text{즉 } V = (n-1)S^2 \sim \chi^2(n-1)
$$
...
**전체 모평균(overall population mean)**
$$
...
전체 표본평균과 전체 모평균은 $\bar X = \frac{1}{k}\sum_{i=1}^k \bar X_i, \quad \bar\mu = \frac{1}{k}\sum_{i=1}^k \mu_i$따라서
$$
...
\right)
$$
특히 $$\hat{\beta}_1 \sim N\left(\beta_1, \frac{\sigma^2}{S_{xx}}\right)$$
