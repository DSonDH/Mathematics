...
연속형의 $k$차원 확률변수 $X=(X_1,\dots,X_k)^T$와 함수
...
>카이제곱분포는 정규분포를 따르는 모집단에서 표본을 추출했을 때 표본분산과 관련된 통계량들이 자연스럽게 따르는 분포입니다. <정리 4.2.2>에 나타난 바와 같이, 정규모집단 $N(\mu,\sigma^2)$에서의 표본분산 $S^2$에 대하여 $(n-1)S^2/\sigma^2$이 $\chi^2(n-1)$을 따릅니다. 이러한 성질은 다음과 같은 통계적 추론을 가능하게 합니다:
>
>- **모분산의 신뢰구간 추정** (<정리 4.2.4>): 모분산에 대한 신뢰구간을 카이제곱분포를 이용하여 구성할 수 있습니다.
>- **분산 동질성 검정**: 두 정규모집단의 분산이 같은지를 검정하기 위해 F분포를 사용하는데, 이는 카이제곱분포의 비율로 정의됩니다 (<정리 4.2.6>).
>- **분산분석(ANOVA)**: 여러 정규모집단의 모평균을 비교할 때, 집단 간 변동과 집단 내 변동이 카이제곱분포를 따르므로, 이들의 비율인 F통계량을 통해 모평균들의 동일성을 효율적으로 검정할 수 있습니다 (<정리 4.2.7>).
...
#### 예 4.2.2
감마분포 $\text{Gamma}(2,3)$의 상방 $0.05$ 분위수는 $3\chi^2_{0.05}(4)/2$ 로서, 부록 IV에 주어진 카이제곱분포의 누적확률 분포표로부터
...
$$\bar X = \frac{1}{k}\sum_{i=1}^k \bar X_i, \quad \bar\mu = \frac{1}{k}\sum_{i=1}^k \mu_i$$
...
또한 $\Sigma=AA^T$이고 $\det(\Sigma)=\det(AA^T)=(\det A)^2$이므로

$$|\det(A)|=(\det(\Sigma))^{1/2} \quad \Rightarrow \quad |\det(A)|^{-1}=(\det(\Sigma))^{-1/2}$$
...
$$
\text{for some s, mgf}_Z(s)=\exp\left(\frac12 s^Ts\right)
$$

따라서 $s=\Sigma^{1/2}t$이므로
...
$$
\Sigma=\text{diag}(\sigma^2/n_1,\dots,\sigma^2/n_k)=\sigma^2D^{-1},\quad D=\text{diag}(n_1,\dots,n_k)
$$

$\mathbf{1}=(1,\dots,1)^T$로 놓으면 $\mathbf{n}=D\mathbf{1}$이고, $n=\sum_{i=1}^k n_i$에 대하여

$$
\bar{X}-\bar{\mu}=\frac{1}{n}\mathbf{n}^T(Y-\mu)=\frac{1}{n}\mathbf{1}^TD(Y-\mu)
$$

따라서 편차벡터는

$$
v:=(Y-\mu)-(\bar{X}-\bar{\mu})\mathbf{1}=\left(I-\frac{1}{n}\mathbf{1}\mathbf{n}^T\right)(Y-\mu)
$$

$A:=I-\frac{1}{n}\mathbf{1}\mathbf{n}^T$로 정의하면 원하는 가중제곱합은

$$
Q:=\sum_{i=1}^k n_i v_i^2 = v^TDv = (Y-\mu)^TA^TDA(Y-\mu)
$$

행렬 $M$을 다음과 같이 정의한다:

$$
M:=A^TDA=\left(I-\frac{1}{n}\mathbf{n}\mathbf{1}^T\right)D\left(I-\frac{1}{n}\mathbf{1}\mathbf{n}^T\right)
$$

그러면 $Q=(Y-\mu)^TM(Y-\mu)$이고, $M$은 대칭행렬이다:

$$
M^T=\left[\left(I-\frac{1}{n}\mathbf{n}\mathbf{1}^T\right)D\left(I-\frac{1}{n}\mathbf{1}\mathbf{n}^T\right)\right]^T=(A^TDA)^T=A^TDA=M
$$

표준화 벡터를 정의하면

$$
Z:=\Sigma^{-1/2}(Y-\mu)=\frac{1}{\sigma}D^{1/2}(Y-\mu)\sim N_k(0,I)
$$

따라서

$$
\frac{1}{\sigma^2}Q=Z^TPZ,\quad P:=D^{-1/2}MD^{-1/2}=D^{-1/2}A^TDAD^{-1/2}
$$

행렬 $P$를 계산하기 위해 다음 핵심 관계식을 보이자:

$$
D^{1/2}AD^{-1/2}=I-\frac{1}{n}ss^T,\quad s:=D^{1/2}\mathbf{1}
$$

이는 $\mathbf{n}=D\mathbf{1}$이므로 $\mathbf{n}^TD^{-1/2}=(D\mathbf{1})^TD^{-1/2}=\mathbf{1}^TD^{1/2}=s^T$에서 다음과 같이 확인된다:

$$
D^{1/2}AD^{-1/2}=D^{1/2}\left(I-\frac{1}{n}\mathbf{1}\mathbf{n}^T\right)D^{-1/2}=I-\frac{1}{n}(D^{1/2}\mathbf{1})(\mathbf{n}^TD^{-1/2})=I-\frac{1}{n}ss^T
$$

따라서

$$
P=D^{-1/2}A^TDAD^{-1/2}=(D^{1/2}AD^{-1/2})^T(D^{1/2}AD^{-1/2})=\left(I-\frac{1}{n}ss^T\right)^T\left(I-\frac{1}{n}ss^T\right)
$$

행렬 $I-\frac{1}{n}ss^T$는 대칭이고 멱등이므로 (여기서 $s^Ts=n$을 사용):

$$
P=\left(I-\frac{1}{n}ss^T\right)^2=I-\frac{1}{n}ss^T
$$

이 행렬 $P$는 다음 성질을 만족한다:
...
$$
P^2=\left(I-\frac{1}{n}ss^T\right)^2=I-\frac{2}{n}ss^T+\frac{1}{n^2}s(s^Ts)s^T=I-\frac{1}{n}ss^T=P
$$

**대각합:**

$$
\text{trace}(P)=\text{trace}(I)-\frac{1}{n}\text{trace}(ss^T)=k-\frac{1}{n}s^Ts=k-1
$$

정리 4.4.5 (b)에 의해

$$
Z^TPZ\sim\chi^2(\text{rank}(P))=\chi^2(k-1)
$$
...
벡터 $X=(X_1,\dots,X_n)^T$에 대하여 $X\sim N_n(\mu\mathbf{1},\sigma^2I), \quad \mathbf{1}=(1,\ldots,1)^T$
...
$$\hat{\beta}_1 = \frac{S_{xy}}{S_{xx}} = \frac{\sum_{i=1}^n (x_i - \bar{x})Y_i}{\sum_{i=1}^n (x_i - \bar{x})^2}, \quad \hat{\beta}_0 = \bar{Y} - \hat{\beta}_1 \bar{x}$$
