...
$$y_i=\beta_1x_{i1}+\cdots+\beta_px_{ip}+\varepsilon_i,\quad \varepsilon_i\sim N(0,\sigma^2),\quad i=1,\dots,n, \quad \boldsymbol{\beta}=(\beta_1,\dots,\beta_p)^T \\
\tag{17.1}$$

$\boldsymbol{\beta}$는 $p$차원의 회귀계수벡터 (regression coefficient vector)이다. 또한 모든 변수는 표준화 (standardization)되어
...
$X^TX$ 가 비정칙행렬 (singular matrix)이 되어 최소제곱추정량 $(X^TX)^{-1}X^Ty$ 이 존재하지 않는다. 정규방정식 (normal equation) $X^TX\boldsymbol{\beta}=X^Ty$ 를 생각하면, 방정식의 수는 $n$개인데 미지수의 수는 $p$개이므로 $p>n$일 때 해가 유일하게 결정되지 않는다.
...
* $D$는 블록행렬 (block matrix)이며, $n>p$일 때
  ...
  $$
  와 같이 쓸 수 있다.
* $D_{p\times p}$의 대각원소 $d_1,\dots,d_p$는 $\mathbf{X}$의 특잇값 (singular values)이다.

직교성에 의해 $UU^T=U^TU=I_n,\quad PP^T=P^TP=I_p$ 가 성립한다.  식 (17.2)로부터

$$ \mathbf{X}^T\mathbf{X}=PD^TU^TUDP^T=PD^TDP^T $$

이므로, $\mathbf{X}^T\mathbf{X}$의 고유값 분해 (eigenvalue decomposition)와의 관계도 알 수 있다. 즉 $\mathbf{X}^T\mathbf{X}$의 고유값은 $d_j^2$이고, $P$는 고유벡터행렬 (eigenvector matrix)이 된다.  
따라서 최소제곱추정량은
...
$$\hat{\boldsymbol{\beta}} = (PD^TDP^T)^{-1}PD^TU^T\mathbf{y} =P(D^TD)^{-1}D^TU^T\mathbf{y} \tag{17.4}$$
...
$\mathbf{X}^T\mathbf{X}$ 에 양의 상수 $k$를 곱한 항등행렬을 더한 다음 역행렬을 취하여 정의한다. 즉 능형회귀추정량은

$$
\hat{\boldsymbol{\beta}}(k)=(\mathbf{X}^T\mathbf{X}+kI_p)^{-1}\mathbf{X}^T\mathbf{y}
\tag{17.5}
$$
...
이제 $p>n$인 고차원 자료를 생각한다. 이 경우 $\mathbf{X}^T\mathbf{X}$의 최대 계수(rank)는 $n$이므로, 정규방정식 $\mathbf{X}^T\mathbf{X}\boldsymbol{\beta}=\mathbf{X}^T\mathbf{y}$ 의 해는 유일하지 않고 무수히 많다. 다시 말해 최소제곱추정량은 일반적으로 존재하지 않거나 유일하지 않다.  
이때 $r(\mathbf{X})=n(<p)$라 가정하면 $D$ 행렬은
...
과 같은 구조를 갖는다. 그러나 능형회귀에서는

$$
(\mathbf{X}^T\mathbf{X}+kI_p)^{-1} =
\sum_{j=1}^p(d_j^2+k)^{-1}\mathbf{p}_j\mathbf{p}_j^T
$$
...
$$|\hat{\boldsymbol{\beta}}(k_1)|<|\hat{\boldsymbol{\beta}}(k_2)|$$
...
$$(E[\hat{\boldsymbol{\beta}}(k)]-\boldsymbol{\beta})^T(E[\hat{\boldsymbol{\beta}}(k)]-\boldsymbol{\beta})
...
실제 고차원 자료에서는 능형회귀를 계산할 때 직접 $(\mathbf{X}^T\mathbf{X}+kI_p)^{-1}$ 를 계산하는 것이 매우 비효율적일 수 있다. 예를 들어 $n=100$, $p=4000$인 마이크로어레이 자료 (microarray data)를 생각하자. 이 경우 $p\times p$ 행렬은 $4000\times 4000$이므로 역행렬 계산 비용이 매우 크다.  
이때 특잇값 분해를 이용하면 더 효율적인 계산이 가능하다. 식 (17.7)에서 특잇값이 0인 부분을 제거하여 $D$와 $P$를 각각 $n\times n$, $p\times n$ 형태로 쓰면, 능형회귀추정량은

$$
\hat{\boldsymbol{\beta}}(k)
= (\mathbf{X}^T\mathbf{X}+kI_p)^{-1}\mathbf{X}^T\mathbf{y}
= (P D^T D P^T+kI_p)^{-1} P D^T U^T \mathbf{y}
= P(D^T D+kI_n)^{-1} D^T U^T \mathbf{y}
$$
...
설명변수 행렬 $X$의 특잇값 분해에서 직교행렬 $P=(\mathbf{p}_1,\dots,\mathbf{p}_p)$를 생각하자. 이때 첫 번째 주성분 (first principal component)은

$$\mathbf{z}_1=X\mathbf{p}_1$$

일반적으로 $g$개의 주성분을 사용할 때, $P$를 $P=(P_g,\;P_{p-g})$ 로 나누면 처음 $g$개의 주성분은 $Z_g=XP_g$ 로 주어진다. 여기서

* $P_g$는 처음 $g$개의 고유벡터 (eigenvectors)로 이루어진 행렬이다.
* $Z_g=(\mathbf{z}_1,\mathbf{z}_2,\dots,\mathbf{z}_g)$는 처음 $g$개의 주성분점수 (principal component scores)를 열로 갖는 행렬이다.

원래의 선형회귀모형 $\mathbf{y}=X\boldsymbol{\beta}+\boldsymbol{\varepsilon}$ 에 대하여 직교변환을 이용하면

$$\mathbf{y}=XPP^T\boldsymbol{\beta}+\boldsymbol{\varepsilon}=Z\boldsymbol{\alpha}+\boldsymbol{\varepsilon}$$

와 같이 표현할 수 있다. 여기서 $\boldsymbol{\alpha}$는 주성분 좌표계에서의 회귀계수벡터이다.  
이제 $g$개의 주성분만을 사용하는 회귀모형 $\mathbf{y}=Z_g\boldsymbol{\alpha}_g+\boldsymbol{\varepsilon}$ 을 가정한다. 즉, 설명변수 전체가 아니라 $X$의 변동을 가장 잘 설명하는 상위 $g$개의 방향만을 선택하여 회귀하는 것이다.
...
$$\hat{\boldsymbol{\beta}}_g=\hat{\boldsymbol{\beta}}$$
...
$$
\mathrm{MSE}(\hat{\boldsymbol{\beta}}_g)
= E\big[(\hat{\boldsymbol{\beta}}_g-\boldsymbol{\beta})^T(\hat{\boldsymbol{\beta}}_g-\boldsymbol{\beta})\big]
$$

이며, 식 (17.12)와 식 (17.14)를 이용하면

$$ \mathrm{MSE}(\hat{\boldsymbol{\beta}}_g) = \sigma^2\sum_{j=1}^{g}\lambda_j^{-1} + \sum_{j=g+1}^{n}(\mathbf{p}_j^T\boldsymbol{\beta})^2 \tag{17.15}
$$
