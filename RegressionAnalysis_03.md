...
> 참고: 비중심 카이제곱분포 표기
>
>  비중심 카이제곱분포는 중심 카이제곱분포에 비중심성 모수 $\lambda$를 추가한 확장 개념이다.
>
> $$
> \chi^2(r(A),\lambda) \quad\text{또는}\quad  \chi^2_{r(A)}(\lambda)
> $$
>
> - 첫 번째 인수 $r(A)$는 자유도(degree of freedom), 즉 행렬 $A$의 계수(rank)다.
> - 두 번째 인수 $\lambda=\frac12\mu^T A\mu$는 비중심성 모수(noncentrality parameter)다.
> - $\mu\neq 0$이면 분포의 중심이 오른쪽으로 이동한다.
>
> 반면 인수가 하나인
>
> $$
> \chi^2(r(A))\quad\text{또는}\quad \chi^2_{r(A)}
> $$
>
> 는 중심 카이제곱분포를 뜻하며,
>
> $$
> \chi^2(r(A)) = \chi^2(r(A),0)
> $$
